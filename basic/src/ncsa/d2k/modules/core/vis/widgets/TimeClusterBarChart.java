...
public class TimeClusterBarChart extends BarChart
  ...
  public TimeClusterBarChart(Table table, DataSet set, GraphSettings settings, int xincrement, int yincrement) throws Exception {
    ...
    //vered replaced following call to reorderRows with a call to copy.
    ...
    for (int row=0; row < rows; row++) {
      ...
      if (!runlabel.equals(label)) {
        ...
        while (iterator.hasNext()) {
          ...
          //vered - addRows is no longer supported in MutableTable.
          //adding the values one by one.
//          mutable.addRow(values);
          mutable.addRows(1);
          int newRowIdx = mutable.getNumRows()-1;
          for (int i=0; i<values.length; i++)
            mutable.setString(values[i], newRowIdx, i);
...
//          mutable.addRow(values);
    ...
    //vered - replaced call to reorderRows witha call to copy
//    mutable = (mutable.reorderRows(indices)).toExampleTable();
  ...
  public String[] getUniqueValues(MutableTable mTable, int col)  {
    ...
    //vered - replacing reorderRows with copy
    mTable = (MutableTable)mTable.copy(indices);
  ...
  */
  ...
  public String[] getLegendTimeRanges(MutableTable mTable) {
    ...
    int[] indices = TableUtilities.multiSortIndex(mutable, sort);
      //vered - replacing reorderRows with copy
    MutableTable sortedTable = (MutableTable) mTable./*reorderRows*/copy(indices);
    int level = 0;
  ...
  //vered - replaced call to getNumEntries with a call to getNumRows.
