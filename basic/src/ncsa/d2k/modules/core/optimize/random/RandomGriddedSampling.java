...
public class RandomGriddedSampling extends RandomSampling {

	/** */
	int [] indices = null;
	
	/** this is the dimension multiplier, contains the cardinality of that level of the space.*/
	long [] multiplier = null;
	
	/** this is the increment, the indexed distance between points, not the actual euclidean distance. */
	double increment;
...
"      displayed when each point is generated."+
"    </p>";
	}
	...
	/**
	 * This is the method that computes the next paramamter point and pushes it out. To do this
	 * the N dimension space is reduced to a 1D space. It is simply a range, starting at the first
	 * point on the line and continuing to the last possible point in the space. This mapping
	 * is easy, that is what the resolution of each parameter in the space provides. Once the 
	 * next equally spaced point is selected, we offset the point by a random value from zero to
	 * the distance to the next point on the line. This provides better coverage than a regularly
	 * spaced grid.
	 */
	protected void pushParameterPoint() {
	   ...
	   if (multiplier == null) {
	   	...
	   	for (int i = numParams-1; i >= 0; i--) {
				multiplier [i] = inttot;
				inttot *= space.getResolution(i);
	   	}
	   	increment = (double)inttot/(double)this.getMaxIterations();
	   }
	   ...
	   for (int i = 0; i < numParams; i++) {
		...
		// Now we have the indices of the increments, compute the floating point value
		// at that interval.
		double[] point = new double[numParams];
	   for (int i = numParams - 1; i >= 0; i--) {
	   	if (space.getResolution(i) == 1)
	   		point[i] = space.getMinValue(i);
	   	else {
		   	point[i] = ((double)indices[i])/(space.getResolution(i)-1.0);
	   ...
	   if (trace) {
		  System.out.println("Equidistant Sampling Pushed point " + pointsPushed + " " + parameterPoint);
	   }
