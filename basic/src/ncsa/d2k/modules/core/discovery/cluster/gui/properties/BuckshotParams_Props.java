...
/**
 * <p>
 * Title: BuckshotParams_Props
 * </p>
 * <p>
 * Description: The Custom Properties Editor GUI for BuckshotParams Module
 * </p>
 * <p>
...
public class BuckshotParams_Props extends JPanel implements CustomModuleEditor,
	 ...
	 * @throws Exception
	 *             If an invalid value is inserted in one of the GUI fields
	 *             (such as a String in a numeric field).
	 */
	public boolean updateModule() throws Exception {
		...
		try {
			...
			throw new PropertyVetoException(
					"Error in number of clusters field: " + e.getMessage(),
					null);
		}
		if (num < 2) {
			throw new PropertyVetoException(
					"Number of clusters must be two or more.", null);
		}
		...
		try {
			...
			throw new PropertyVetoException(
					"Error in number of assignments field: " + e.getMessage(),
					null);
		}
		if (maxit < 1) {
			throw new PropertyVetoException(
					"Number of assignments must be > 1.", null);
		}

		if (_src != null) {
			_src.setUseFirst(m_useFirst.isSelected());
			if (m_auto.isSelected()) {
				_src.setDistanceThreshold((m_distSlide.getValue() == 0) ? 1
	...
	/**
	 * Instantiates the GUI components of the editor.
	 * 
	 */
	private void init() {
		setLayout(m_gbl);
		...
		m_methods.setToolTipText(HAC.s_ClusterMethodDesc[_src
				.getClusterMethod()]);
		...
		m_gbc.gridx = 1;
		m_gbc.anchor = GridBagConstraints.WEST;
		m_seed = new JTextField(Integer.toString((_src.getSeed() < 0) ? 0
				: _src.getSeed()), 5);
		...
		m_numClustLbl
				.setToolTipText("Enter integer value > 2 specifying number"
						+ " of clusters desired.");
		m_gbl.setConstraints(m_numClustLbl, m_gbc);

		m_gbc.gridx = 1;
		m_gbc.anchor = GridBagConstraints.WEST;
		m_numClust = new JTextField(Integer
				.toString((_src.getNumClusters() < 2) ? 5 : _src
						.getNumClusters()), 5);
		...
		m_distLbl.setText("Distance Cutoff (% of Max):  "
				+ Integer.toString((_src.getDistanceThreshold() == 0) ? 1
						: _src.getDistanceThreshold()));
		...
		m_gbc.anchor = GridBagConstraints.CENTER;
		m_gbc.insets = new Insets(2, 2, 2, 2);
		m_distSlide = new JSlider(JSlider.HORIZONTAL, 0, 100, (_src
				.getDistanceThreshold() == 0) ? 1 : _src.getDistanceThreshold());
		...
		m_distMetrics.setToolTipText(HAC.s_DistanceMetricDesc[_src
				.getDistanceMetric()]);
		...
		m_gbc.gridx = 1;
		m_gbc.anchor = GridBagConstraints.WEST;
		m_max = new JTextField(Integer
				.toString((_src.getMaxIterations() < 1) ? 5 : _src
						.getMaxIterations()), 5);
	...
	/**
	 * Called when the user is manipulating the combo boxes int he GUI
	 */
	public void actionPerformed(ActionEvent e) {
		Object src = e.getSource();
		if (src == this.m_methods) {
			JComboBox cb = (JComboBox) src;
			m_methods.setToolTipText(HAC.s_ClusterMethodDesc[cb
					.getSelectedIndex()]);
		}
		if (src == this.m_distMetrics) {
			JComboBox cb = (JComboBox) src;
			m_distMetrics.setToolTipText(HAC.s_DistanceMetricDesc[cb
					.getSelectedIndex()]);
		}
	}
	...
	public void stateChanged(ChangeEvent e) {
		if (e.getSource() == m_distSlide) {
			String disp = Integer.toString((m_distSlide.getValue() == 0) ? 1
					: m_distSlide.getValue());
