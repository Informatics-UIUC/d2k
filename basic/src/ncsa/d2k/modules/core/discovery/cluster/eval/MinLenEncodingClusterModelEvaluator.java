...
public class MinLenEncodingClusterModelEvaluator extends ComputeModule {
	...
	/**
	 * If true this module perform in verbose mode and outputs information to
	 * stdout
	 */
	 ...
	 * 
	 * @param b
	 *            boolean If true, this setter method puts this module in
	 *            verbose mode.
	 */
	...
	public PropertyDescription[] getPropertiesDescriptions() {
		...
		descriptions[0] = new PropertyDescription("verbose", "Verbose Output",
	...
	public String getModuleInfo() {
		String s = "<p>Overview: ";
		s += "This module outputs a score for the input cluster model using a "
			+"minimum ";
		s += "encoding length measurement.";
		s += "</p>";

		s += "<p>Detailed Description: ";
		s += "The minimum encoding length measure is affected by the number of "
			+"clusters ";
		s += "formed and the amount of information that can be saved by tagging "
			+"cluster ";
		s += "entities with a cluster ID.  The intuition is that good "
			+"clusterings ";
		s += "will form clusters that meaningfully limit the possibility of "
			+"values thus ";
		s += "economizing the amount of information needed to describe the "
			+"overall ";
		s += "clustering.";
		s += "</p>";
		s += "<p>";
		s += "Reference: M.-C. Ludl and G. Widmer. Towards a Simple Clustering "
			+"Criterion ";
		s += "Based on Minimum Length Encoding. "
		...
		s += "<p>Scalability: ";
		s += "This algorithm makes two passes over the table that is part of "
			+"the cluster ";
	...
	/**
	 * Calaulates the score o fthe input cluster model's data.
	 */
	public void doit() throws Exception {
		try {
			...
			ParameterPoint objectivePoint = ParameterPointImpl
					.getParameterPoint(names, utility);
