...
public class RandomSearch extends ASSearch
 ...
 /** does the data have a class */
   ...
   */
   public int[] search (ASEvaluation ASEval, Table data)
	 ...
	 if ((m_starting[i]) != m_classIndex) {
       ...
       m_bestGroup = generateRandomSubset(inputFeatures);
       best_merit = ASEvaluator.evaluateSubset(m_bestGroup);
     }

			if(m_verbose) System.out.println("M_verbose set to true");
		   else  System.out.println("M_verbose set to false");
     if (m_verbose) {
     ...
     for (i=0;i<m_iterations;i++) {
       temp = generateRandomSubset(inputFeatures);
   ...
   */
  private BitSet generateRandomSubset(int [] inputFeatures) {
	...
	//if (m_hasClass && i == m_classIndex) {
		if (m_hasClass && inputFeatures[i] == m_classIndex) {
	} else {
	  temp.set(inputFeatures[i]);
	}
      }
    }
    return temp;
