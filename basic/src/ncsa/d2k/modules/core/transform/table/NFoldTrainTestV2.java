...
public class NFoldTrainTestV2 extends ncsa.d2k.core.modules.DataPrepModule {
	...
	/** the data table. */
	ParameterPoint parameterPoint = null;
	...
	public String getInputInfo(int i) {
		...
		case 1:
			return "The paramter point to use for controlling the learning algorithm which is given the train table.";
		default:
	...
	public String[] getInputTypes() {
		String[] types = { "ncsa.d2k.modules.core.datatype.table.Table", "ncsa.d2k.modules.core.datatype.parameter.ParameterPoint" };
		return types;
	}
	...
	public PropertyDescription[] getPropertiesDescriptions() {
		...
		pds[0] = new PropertyDescription("numberFolds", "Number of Folds",
				"The number of folds for the cross validation. This number must be greater than 2.");
		pds[1] = new PropertyDescription("seed", "Seed",
		...
		pds[2] = new PropertyDescription("debug", "Verbose output",
	...
	public void doit() throws Exception {
		...
		if (numFires == 0) {
			...
			parameterPoint = (ParameterPoint) this.pullInput(1);
			setup();
			this.pushOutput(new Integer(Nfolds), 3);
		}
		...
		System.arraycopy(indices, breaks[numFires + 1], training, breaks[numFires], numRows - breaks[numFires + 1]);

		if (debug) {
			System.out.println(getAlias() + ": Fold " + numFires);
			System.out.println(getAlias() + ": Test Set size:  " + testSize + " and indicies: ");
			for (int j = 0; j < testSize; j++) {
				System.out.println(testing[j]);
			}

			System.out.println(getAlias() + ": Train Set size:  " + trainSize + " and indicies: ");
	...
	protected void setup() throws Exception {
		...
		if (numRows < Nfolds) {
			throw new Exception(this.getAlias() + ": The input table must not contain fewer rows than the number of folds requested. \n"
