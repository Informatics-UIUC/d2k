...
/*	String fillerString = new String("*");
	...
	public String getModuleName() {
        ...
        /**
         * sets the missing values flags in <code>dest</codE>, according to <code>source</code>.
         * <code>start</code> and  <code>end</code> are indices into the flags array of  <code>dest</code>.
         * <code>end</code> should be greater than the last flag's idex to be modified, by one.
         * @param source - source column. if null all flags as set to true.
         * @param dest - destination column.
         * @param start - start index into <codE>dest</code>
         * @param end - end index into <codE>dest</codE>.
         *              should be greater than the last flag's idex to be modified, by one.
         */
        private void setMissingValues(Column source, Column dest, int start, int end){
          if(source == null)
            for (int i=start; i<end; i++)
              dest.setValueToMissing(true, i);
          else
            for(int i=start, j=0; i<end && j<source.getNumRows(); i++, j++)
              dest.setValueToMissing(source.isValueMissing(j), i);
        }
	 ...
	 */
	private void copyMissingValues(Column source, Column dest, int start, Table tbl, int type) {
		if (source == null) {

			// if there is no source column, verything is missing.
			for (int i = start; i < dest.getNumRows(); i++) {
				switch (type) {
			...
			for (int i = 0; i < t1numColumns; i++) {
				...
				switch (t1.getColumnType(i)) {
					case ColumnTypes.INTEGER :
						{
							...
							System.arraycopy(
								(int[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
							...
							if (t2Col != -1) {
								System.arraycopy(
									(int[]) t2.getColumn(t2Col).getInternal(),
									0,
									vals,
									t1Size,
									t2Size);
							...
							this.copyMissingValues(t1.getColumn(i), col, 0, t1, t1.getColumnType(i));
							if (t2Col != -1)
								this.copyMissingValues(t2.getColumn(t2Col), col, t1Size, t1, t1.getColumnType(i));
							else
								this.copyMissingValues(null, col, t1Size, t1, t1.getColumnType(i));
					...
					case ColumnTypes.FLOAT :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(float[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
							...
							if (t2Col != -1) {
								System.arraycopy(
									(float[]) t2.getColumn(t2Col).getInternal(),
									0,
									vals,
									t1Size,
									t2Size);
					...
					case ColumnTypes.DOUBLE :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(double[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
					...
					case ColumnTypes.SHORT :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(short[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
							...
							if (t2Col != -1) {
								System.arraycopy(
									(short[]) t2.getColumn(t2Col).getInternal(),
									0,
									vals,
									t1Size,
									t2Size);
					...
					case ColumnTypes.LONG :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(long[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
							...
							if (t2Col != -1) {
								System.arraycopy(
									(long[]) t2.getColumn(t2Col).getInternal(),
									0,
									vals,
									t1Size,
									t2Size);
					...
					case ColumnTypes.STRING :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(String[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
					...
					case ColumnTypes.CHAR_ARRAY :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(char[][]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
					...
					case ColumnTypes.BYTE_ARRAY :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(byte[][]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
					...
					case ColumnTypes.BOOLEAN :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(boolean[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
					...
					case ColumnTypes.OBJECT :
						{
							...
							// get the data from table 1, put it first.
							System.arraycopy(
								(Object[]) t1.getColumn(i).getInternal(),
								0,
								vals,
								0,
								t1Size);
					...
					case ColumnTypes.BYTE :
						{
							...
							if (t2Col != -1) {
								System.arraycopy(
									(byte[]) t2.getColumn(t2Col).getInternal(),
									0,
									vals,
									t1Size,
									t2Size);
					...
					case ColumnTypes.CHAR :
						{
							...
							if (t2Col != -1) {
								System.arraycopy(
									(char[]) t2.getColumn(t2Col).getInternal(),
									0,
									vals,
									t1Size,
									t2Size);
					...
					default :
						{
							throw new Exception(
								"Datatype for Table 1 column named '"
									+ label
		...
		tbl2cols : for (int i = 0; i < t2numColumns; i++) {
			...
			switch (t2.getColumnType(i)) {
				...
				case ColumnTypes.INTEGER :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingInt();//(int) this.getFillerNumeric();
						}
						...
						// get the data from table 2, append it
						System.arraycopy(
							(int[]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
						...
						/*this.setMissingValues(null, col, 0, t1.getNumRows());
						this.setMissingValues(t2.getColumn(i), col, t1.getNumRows(), col.getNumRows());
						result.addColumn(col);*/
						break;
					}
				...
				case ColumnTypes.FLOAT :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = (float) t1.getMissingDouble();//(float) this.getFillerNumeric();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
						...
						// get the data from table 2, append it
						System.arraycopy(
							(float[]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
						...
						//vered - added following 2 rows to preserve missing values of unique columns in second table (bug 108)
						/*Column */col = new FloatColumn(vals);
                                                //vered - replaced the following lines with calls to setMissingValues
                                                //right after end of this switch block.
						/*this.copyMissingValues(null, col, 0, t1, t1.getColumnType(i));
						this.copyMissingValues(t2.getColumn(i), col, t1.getNumRows(), t1, t1.getColumnType(i));
						result.addColumn(col);*/
				...
				case ColumnTypes.DOUBLE :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingDouble();// (double) this.getFillerNumeric();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
						...
						// get the data from table 2, append it
						System.arraycopy(
							(double[]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
				...
				case ColumnTypes.SHORT :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = (short)t1.getMissingInt();//(short) this.getFillerNumeric();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
						...
						// get the data from table 2, append it
						System.arraycopy(
							(short[]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
				...
				case ColumnTypes.LONG :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = (long) t1.getMissingInt();//this.getFillerNumeric();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
				...
				case ColumnTypes.STRING :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingString();//this.getFillerString();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
						...
						// get the data from table 2, append it
						System.arraycopy(
							(String[]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
				...
				case ColumnTypes.CHAR_ARRAY :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingChars();//this.getFillerChars();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
						...
						// get the data from table 2, append it
						System.arraycopy(
							(char[][]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
				...
				case ColumnTypes.BYTE_ARRAY :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingBytes();//this.getFillerBytes();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
						...
						// get the data from table 2, append it
						System.arraycopy(
							(byte[][]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
				...
				case ColumnTypes.BOOLEAN :
					{
						...
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingBoolean();//this.getFillerBool();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
						...
						// get the data from table 2, append it
						System.arraycopy(
							(boolean[]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
				...
				case ColumnTypes.OBJECT :
					{
						...
						// get the data from table 2, append it
						System.arraycopy(
							(Object[]) t2.getColumn(i).getInternal(),
							0,
							vals,
							t1Size,
							t2Size);
				...
				case ColumnTypes.BYTE :
					{
						...
						byte[] s1 = new byte[t1Size];
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingByte();//this.getFillerByte();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
				...
				case ColumnTypes.CHAR :
					{
						...
						char[] s1 = new char[t1Size];
						for (int j = 0; j < t1Size; j++) {
							s1[j] = t1.getMissingChar();//this.getFillerChar();
						}
						System.arraycopy(s1, 0, vals, 0, t1Size);
						s1 = null; // hint to gc
				...
				default :
					{
						throw new Exception(
							"Datatype for Table 2 column named '"
								+ label
								+ "' was not recognized when appending tables.");
					}
			}//switch
