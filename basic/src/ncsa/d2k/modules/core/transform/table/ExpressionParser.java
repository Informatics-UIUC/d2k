 ...
 * also operator 'is' is a valid sql opertor
...
public class ExpressionParser {
  ...
  private HashMap attMap; //this will hold the legal left hand operand.
  private String[] operators; //this will hold the legal operators.
   private String[] relations; //this will hold the legal relations.
  private String delimiter = " \n\t\f\r()<>=!&|"; //will hold the legal delimiters int he expression.
  private boolean sql;
 private HashMap opsMap ;
 private HashMap relationMap;
 private boolean[] isScalar; //for each of the attributes in the attMap, indicates if it is scalar (true) or nominal (false)

  public ExpressionParser() {
  }

  public ExpressionParser(HashMap map, String[] ops, String[] relate, boolean[] scalarFlags, boolean isSql, String delim) {
    this(map, ops, relate, scalarFlags, isSql);
    delimiter = delim;

  }

  public ExpressionParser(HashMap map, String[] ops, String[] relate, boolean[] scalarFlags, boolean isSql ) {
   attMap = map;
   operators = ops;
   sql = isSql;
   relations = relate;
     ...
     StringTokenizer tok = new StringTokenizer(expression, " \n\t\f\r()<>=!&|", true);
      ...
      // String lastGoodToken = ""; //in cased we've parsed a non relative sub expression
       ...
       while(tok.hasMoreTokens()){
         ...
         if(sql){
           if(currTok.equals(and) || currTok.equals(or) || currTok.equals(is)){
 ...
 //           if(currTok.length() > 1) throw new Exception("there must be a space before and after each parentheses!");
     ...
     //if this is not an sql parser - parsing the next token, expecting to find a delimiter '&' or '|'
...
*/
    ...
    return goodCondition;
      ...
      /**
       * parses <code>expression</code> into sub expression, to verify
       * that all of them are legal and relevant to the attributes in <code>attMap</code>.
       * allows empty parentheses. checks for validity of parentheses as well.
       * this method is static becuase it supports also sql filter construction module's doit.
       * @param expression - an expression to parse.
       * @return -    <code>expression</code> - if is valid
       * @throws Exception - if finds an unexpected token or illegal prentheses or an attribute that is not in attMap.
       */
     ...
     /*   final String equal, and, or, is;
        ...
        StringTokenizer tok = new StringTokenizer(expression, delimiter/*" \n\t\f\r()<>=!&|"*/, true);
           ...
           if(!currTok.equals(secondChar))
            ...
            case '=':  //are we expecting a relation?
                       if(!(expected == Expected.RELATION))
                         throw new Exception ("Did not expect to find relation " + currTok + " after " + goodCondition +
                                      " " + leftHand + " " + relation + " " + rightHand);
//special case, we are expecting a relation and it is an sql relation. so equal is a sigle '='
                       if(sql){
                         //the relation will be added if the sub expression is relevant and valid.
                      relation = currTok;
                      expected = Expected.AFTER_RELATION;
            ...
            case '>':
             ...
             if(c == '=')
              ...
              //if it is not a white space - it is illegal.
                ...
                //secondChar holds the next token which is not white space, so it has to be right hand operand.
                relation = currTok;
                currTok = secondChar;
                getNext = false;
                expected = Expected.AFTER_RELATION;
            ...
            default: //meaning this is either left or right hand operands or the relation "is"
          ...
          if(sql && Expected.RELATION == expected){
            ...
            else throw new Exception("Found illegal relation: " + currTok);
   ...
   */
      ...
      private void validateSmallExpression(String leftHand, String rightHand, String relation)throws Exception{
        String exp = leftHand + " " + relation + " " + rightHand;

        if(!attMap.containsKey(leftHand)){
          throw new Exception("Found illegal expression: " + exp +
                              " non of the operands " + leftHand + " or " + rightHand +
        ...
        if(!attMap.containsKey(rightHand)){
          ...
          //if left hand is a scalar attribute, making sure we can parse the right hand value
          if(isScalar[index]){
            try{
              ...
              throw new Exception ("Found illegal expression: " + exp +
                                   "\n" + leftHand + " is a scalar attribute but " +
            ...
            if(rightHand.charAt(0) != '\'' || rightHand.charAt(rightHand.length() -1) != '\'')
              throw new Exception  ("Found illegal expression: " + exp +
                                   "\n" + leftHand + " is a nominal attribute but " +
                                   rightHand + " is not surrounded by single quote marks.");
