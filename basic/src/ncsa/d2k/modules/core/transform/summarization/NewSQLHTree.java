 ...
 *
 * <p>Description: Calculate a Cube from a database table based on HTree
 * algorithm</p>
 *
 ...
 * @version 1.0
 */
public class NewSQLHTree extends ComputeModule implements java.io.Serializable {

   //~ Static fields/initializers **********************************************

   /** Use serialVersionUID for interoperability. */
   static private final long serialVersionUID = 4326588778323951441L;
   ...
   static private String ELN = "\n";

   /** constant */
   static private String NA = "~~~~";

   /** constant for maximum table name length */
   static private int MAXTABLENAME = 30;

   //~ Instance fields *********************************************************

   /** ArrayList of LocalHead for localHeadTbls. */
   private ArrayList aLocalHeadTbl;

   /** ArrayList of Head for base header table. */
   private ArrayList baseHeadTbl;
   ...
   private ArrayList hTree;

   /** ArrayList of ArrayList for local head tables. */
   private ArrayList localHeadTbls;

   /** maximum number of columns that can be included for analysis. */
   private int maxColumn = 25;

   /** maximum rule size */
   private int maxRuleSize = 5;

   /**
    * allowed maximum ratio of the number of uniq values over the total number
    * of rows for each column. Binning is required if it is over.
    */
   private double maxUniqRatio = 0.3;

   /** integer array for prefix list. */
   private int[] prefixList;
   ...
   /** where clause used in query */
   private String whereClause;

   //~ Methods *****************************************************************

   /**
    * The column name may contain a table name as the prefix. This method
    * removes the prefix, and returns the column name only
    *
    ...
    * the names of two columns. If two column names are only different on the
    * prefix, they reference to the same column.
    *
   ...
   private boolean isSameColumn(String colName1, String colName2) {
      ...
      // System.out.println("colNames in isSameColumn are " + colName1 + ", " +
      // colName2);
   ...
   /**
    * Determine whether the column belongs to the table based on the prefix of
    * the column name.
    *
    * @param  column String
    * @param  table  String
      ...
      try {
         ...
         for (int colIdx = 0; colIdx < columnList.size(); colIdx++) {
            ...
            if (columnStr.equals(NOTHING)) {
               ...
               columnStr =
                  columnStr + ((Column) columnList.get(colIdx)).columnName;
            } else { // add comma between column names

               columnStr =
                  columnStr + ", " +
                  ((Column) columnList.get(colIdx)).columnName;
            }
         ...
         while (tableSet.next()) {
            ...
            for (int colIdx = 0; colIdx < columnList.size(); colIdx++) {
               ...
               endIdx = startIdx + aColumn.uniqCnt; // don't subtract 1,
                                                    // because need to add one
                                                    // extra item for NA
               ...
               if (!aColumn.isBinned) {
                  ...
                  // To speed up the search, first find the starting index and
                  // ending index

                  // in baseHeadTbl for this column.

                  itemIdx =
                     getIndex(startIdx, endIdx, aValue, false,
                              aColumn.dataType);
                  ...
                  itemIdx = endIdx; // NA value is always the last item for the
                                    // column

               } else if (
                          aColumn.isBinned &&
                             aColumn.dataType.equals("STRING")) {

                  aValue = binValue(colIdx, aValue);

                  itemIdx =
                     getIndex(startIdx, endIdx, aValue, true, aColumn.dataType);
                  ...
                  if (itemIdx < 0) {
                     ...
                     return false;
                          ...
                          aColumn.isBinned &&
                             aColumn.dataType.equals("NUMBER")) {

                  aValue =
                     binValue(colIdx, Double.valueOf(aValue).doubleValue());
   ...
   } // end method buildHTree


   /**
    * compute the localHeadTbl against a value in the level with hTreeLevel. The
    * hTreeLevel is equivalent to the column index, because one column goes to
    ...
    * @param localHeadTblsIdx the index of localHeadTblsIdx. This determine
    *                         which localHeadTbl to go. 0 for 2-item set, 1 for
    *                         3-item set, ..., etc
    */
   protected void computeItemSet(int headIdx, int hTreeLevel,
                                 int localHeadTblsIdx) {
      ...
      while (firstNode != -1) {
         ...
         while (parentTreeIdx > 0) { // 0 is the root
            ...
            for (int localIdx = 0; localIdx < localHeadTbl.size(); localIdx++) {
               ...
               if (
                   ((LocalHead) localHeadTbl.get(localIdx)).headIdx ==
                      parentHeadIdx) {
      ...
      if (hTreeLevel > 1 && localHeadTblsIdx < (ruleSize - 2)) {
         ...
         for (int colIdx = 1; colIdx < (hTreeLevel); colIdx++) {
            ...
            // reset the last valid item in prefixList because the recursion on
            // this level should stop.

            // The prefixList should be ready for recursion of one level up.

            for (
                 int prefixIdx = prefixList.length - 1;
                    prefixIdx >= 0;
                    prefixIdx--) {
   ...
   } // end method computeItemSet


   /**
    * Compute every node in the hTree, starting from level 2 to max level.
    */
   ...
   } // end method computeTree


   /**
    * create a database table to save the cube data.
    *
    * @return create a database table to save the cube data.
    */
   protected boolean createCubeTable() {
      ...
      try {
         ...
         // To support concurrent user environment, include user's OS login and
         // DB login in cube table name

         String userDBName =
            cw.getConnection().getMetaData().getUserName().toUpperCase();
         ...
         // to support multi-table operation, only use the first table name to
         // construct the cube table name.
         ...
         if (foundTable(cubeTableName)) {
            ...
            Object tempObj =
               JOptionPane.showInputDialog(inputBoard,

               " The cube table " + cubeTableName +
            ...
            if (tempObj == null) {
               ...
               "The cube table cannot be created because the cube table name is not specified.",
                                             "Error",
         ...
         // Create an empty cube table. The table has the selected columns +
         // count column
         ...
         for (int i = 0; i < tableList.length; i++) {
            ...
            // System.out.println("tableList " + i + " is " + tableList[i] +
            // "zzz");
            ...
            while (columns.next()) {
               ...
               if (isChosenColumn(tableList[i], cname)) {
                  ...
                  // A binned Categorical column may have a value that is
                  ...
                  if (ctype.indexOf("CHAR") >= 0) {
                     ...
                     // determine the column size using the max length of
                     // binned value.
         ...
         // add two columns to the cube table, one column for saving the size
         // of the item set, another for saving counts
         ...
         // insert a record to the cube table. This record is the most
         // aggregated record. There is

         // no value for all columns except the column cnt which is the total
         // number of rows in the

         // data set.

         sb =
            new String("insert into " + cubeTableName + " (CNT) values (" +
                       totalRow + ")");
   ...
   } // end method createCubeTable


   /**
    * Create an ArrayList to hold all local header tables Each localHeadTbl for
    * an item set: [0] for 2-item set, [1] for 3-item set, ..., etc.
    */
   ...
   } // end method createLocalHeadTbls
   ...
   /**
    * Find TABLE_NAME in metadata
    *
    * @param  aCubeTable name of cube table
    *
    * @return true if TABLE_NAME is found
    */
   protected boolean foundTable(String aCubeTable) {
      ...
      try {
         ...
         ResultSet tableNames =
            metadata.getTables(null, "%", aCubeTable, types);
   ...
   protected int getCubeRowCount(String table) {
      ...
      try {
         ...
         String sb =
            new String("select count(*) from " + table +
   ...
   protected int getIndex(int first, int last, String val, boolean isBinned,
      ...
      // use binary search to find the value's index for non binned categorical
      // column
      ...
      } else { // use sequencial search to find the value's index for the
               // binned column or the numeric column
   ...
   /**
    * This method generates an array to store multiple table names which comes
   ...
   protected int getUniqCount(int col) {
      ...
      // this is not a binned column
         ...
         try {
            ...
            String sb =
               new String("select count(distinct " + fieldNames[col] +
                          ") from " + tableName);
            ...
            // if uniqCount/totalRow > maxUniqRatio, inform users to binning
            // the column

            if (
                (double) uniqCount / (double) totalRow <= maxUniqRatio ||
                   uniqCount < 100) {
   ...
   protected void initBaseHeadTbl() {
      ...
      try {
         ...
         while (colIdx < columnList.size()) {
            ...
            } else { // this column is not binned, get the unique value from
                     // the database table

               String valueQry =
                  new String("select distinct " + colName + " from ");

               if (whereClause != null && whereClause.length() > 6) {
                  valueQry =
                     valueQry + tableName + " where (" + whereClause +
                     ") and " +
                     colName + " is not null order by " + colName;
               } else {
                  valueQry =
                     valueQry + tableName + " where " + colName +
                     " is not null order by " + colName;
               }

               // System.out.println("in initBaseHeadTbl, valueQry is " +
               // valueQry);
   ...
   protected void initLocalHeadTbl(int localTblIdx) {
      ...
      for (int prefixIdx = 0; prefixIdx < prefixList.length; prefixIdx++) {
         ...
         if (prefixList[prefixIdx] >= 0) {
            ...
            int columnIndex =
               ((Head) baseHeadTbl.get(prefixList[prefixIdx])).columnOrder;
   ...
   } // end method initLocalHeadTbl


   /**
    * Initialize the prefix list For 2-item set, the prefix is 1 item, and for
    * 3-item set, the prefix is 2 items, etc ...
    */
   ...
   /**
    * Return true if columnName is in the columnList
    *
   ...
   protected boolean isChosenColumn(String tableName, String columnName) {
      ...
      for (int colIdx = 0; colIdx < columnList.size(); colIdx++) {
         ...
         if (
             isSameTable(((Column) columnList.get(colIdx)).columnName
                            .toUpperCase(), tableName) &&
                isSameColumn(((Column) columnList.get(colIdx)).columnName
   ...
   protected boolean isColumnNumeric(int col) {
      ...
      try {
         ...
         for (int i = 0; i < tableList.length; i++) {
            ResultSet columns =
               metadata.getColumns(null, "%", tableList[i], "%");
   ...
   /**
    * Print out contents for debugging.
    */
    ...
    * Print debugging info
    *
    * @param localHeadTable head table
    */
   ...
   /**
    * reset counts in hTree for the nodes which are in columns less than
    * columnIdx.
    *
      ...
      for (int headIdx = baseHeadTbl.size() - 1; headIdx > 0; headIdx--) {
         ...
         if (aHead.valueCnt >= cutOff) {
            ...
            try {
               ...
               if (insertStr.indexOf(NA) < 0) { // if any column value contains
                                                // NA, not save it
   ...
   } // end method saveBaseHeadTbl


   /**
    * Save local head table
    *
    * @param localTblsIdx index
    */
   protected void saveLocalHeadTbl(int localTblsIdx) {
      ...
      for (int localIdx = 0; localIdx < localHeads.size(); localIdx++) {
         ...
         if (aLocalHead.valueCnt >= cutOff) {
            ...
            queryStr =
               insertStr1 + ", " + getColumnNameOnly(newColName) +
               ", set_size, cnt) " + insertStr2 + ", '" +
               aLocalHead.value + "', " +

               (localTblsIdx + 2) + ", " + // localTbls[0] for 2-item set, [1]
                                           // for 3-item set
               aLocalHead.valueCnt + ")";
    ...
    *
    * @return  create the object columnList that keeps the information for each
    *         column: column name, the count of uniq values, the starting index
    *         in head table, the data type, binned or not.
    */
   protected boolean setColumnList() {

      Column aColumn;

      // if a user has selected more than maxColumn, return false and quit the
      // program
   ...
   } // end method setColumnList

   /**
    * order the columns based on cardinality. This will put the column with less
    * unique values to the higher level in the HTree. This is important for the
    * efficiency.
    */
   protected void sortColumnList() {
      ...
      for (int i = 0; i < columnList.size(); i++) {
         ...
         for (int j = columnList.size() - 1; j > i; j--) {
            ...
            if (
                ((Column) columnList.get(j - 1)).uniqCnt >
      ...
      // set the starting index in baseHeadTbl for each column to speed up
      // search
      ...
      for (int colIdx = 0; colIdx < columnList.size(); colIdx++) {
         ...
         startIdx = startIdx + aColumn.uniqCnt + 1; // one extra item for value
                                                    // NA

      }

   } // end method sortColumnList
   ...
   protected void updFirstChild(int treeIdx, int parentIdx) {
      ...
      // if parent has no first-child, update the index of first-child for the
      // parent node
   ...
   protected int updLinks(int hdIdx, int pIdx) {
      ...
      // chain of same-value-node. If parent is same, update count in hTree. If
      // no
   ...
   /**
    * add the index of the head node to the prefix list after the computation of
    ...
    * hTree to update the Same Value index for the immediate previous node of
    * same value.
    *
    * @param treeIdx the current node's index in hTree
    * @param headIdx the current node's index in baseHeadTbl
   ...
   public void doit() throws Exception {
      ...
      if (tableName.indexOf("_CUBE") >= 0) {
    	  myLogger.debug("The user has selected a cube to build a new cube.");
      ...
      if (totalRow > 0 && rightInput) {
         ...
         if (setColumnList()) {
            ...
            // to avoid exponential computation, the maxRuleSize is only
            // allowed up to 5
            ...
            // if number of selected columns <= maxruleSize, ruleSize = # of
            // selected column - 1,
            ...
            if (buildHTree()) {
               ...
               if (createCubeTable()) {
                  ...
                  if (getCubeRowCount(cubeTableName) > 0) {
                     ...
                     "There is no rule discovered in the data set. You may like to adjust " +
                        ...
                        "Minimum Support in NewSQLHTree module, and run again.",
                                                   "Error",
               ...
               JOptionPane.ERROR_MESSAGE);
               myLogger.debug("fail to build a HTree.");
    ...
    * @return maxRuleSize
    */
   public int getMaxRuleSize() { return maxRuleSize; }
   ...
   public String getModuleInfo() {
      ...
      s +=
         "This module creates a cube representation for data in a database table. ";


      s += "</p><p>Detailed Description: ";

      s +=
         "This module constructs a data cube table from selected data in a database table, optionally applying ";

      s +=
         "data filtering and grouping operations on the original data prior to building the cube. ";
      ...
      s +=
         "A cube is an efficient representation used to maintain statistics for itemsets ";

      s +=
         "that appear frequently in table data. An itemset is a collection of items, and an ";

      s +=
         "item is an [attribute, value] pair that is found in the table data.  An itemset is ";

      s +=
         "considered frequent if the items in the itemset occur together in more than a minimum ";
      ...
      s +=
         "The user sets the minimum percentage threshold via the <i>Minimum Support % </i>property.  ";

      s +=
         "If an itemset has support of 12%, then 12% of the data under analysis includes ";

      s +=
         "the particular collection of [attribute,value] pairs that make up the itemset. ";

      s +=
         "The goal is to set the <i>Minimum Support</i> value such that important relationships are captured ";

      s +=
         "without using excessive resources and computing statistics for collections of items that only rarely ";

      s += "occur together. ";

      s +=
         "The user can also control the maximum number of items included in any itemset via the ";

      s +=
         "<i>Maximum Items per Itemset</i> property.  To avoid excessive computation, ";

      s +=
         "a general guideline is that no more than five items should be included in any itemset. ";


      s += "</p><p>";

      s +=
         "This module takes five inputs that determine the data to be included in the cube ";

      s += "representation and the optional filtering and grouping operations. ";

      s +=
         "The first three inputs specify a database connection, a database table ";
      ...
      s +=
         "<i>Connect To DB</i>, <i>Select Table</i>, and <i>Select Attributes</i> modules. ";
      ...
      s +=
         "The input <i>Query Condition</i> specifies how the data should be subset or filtered. ";

      s +=
         "When this input port is used, it is typically linked to the output port of a <i>SQL Filter Construction</i> module. ";

      s +=
         "The input <i>Binning Transformation</i> specifies how attribute values should be grouped. ";

      s +=
         "For attributes that have many distinct values, especially for continuous ";

      s += "numeric attributes, grouping values into bins helps reduce ";

      s +=
         "computation and memory requirements. In addition, grouping the data can lead to insights ";

      s +=
         "that may not be apparent when each unique values is considered independently. ";

      s +=
         "The module <i>SQL Bin Columns</i> specifies the binning operations, and its output port ";

      s +=
         "is typically linked to the <i>Binning Transformation</i> input port when grouping is desired. ";


      s += "</p><p>";

      s +=
         "At runtime this module makes a connection to a database, retrieves the ";
      ...
      s +=
         "Once a cube is built, it can be used to quickly perform further analysis on the data.  ";

      s +=
         "Rules can be extracted from the cube that have a <i>Minimum Support %</i> greater than or equal to the ";

      s +=
         "support level used to build the cube.  Any rule built from the cube will contain at most ";
      ...
      s +=
         "Due to the intense computation involved in constructing a data cube, the memory and CPU requirements ";

      s +=
         "are substantial when the input data contains numerous attributes and many unique values. ";

      s +=
         "In general, it is recommended that the input data have fewer than 25 attributes. ";

      s +=
         "It is also recommended that data be grouped into bins for attributes with large numbers of unique data values. ";

      s +=
         "If the number of unique values for an attribute is greater than 5% of the total ";

      s +=
         "number of records, execution stops with a recommendation that the binning should be performed. ";
    ...
    */
   public String getModuleName() { return "SQL HTree"; }
   ...
   public PropertyDescription[] getPropertiesDescriptions() {
      ...
      pds[1] =
         new PropertyDescription("support",
   ...
   public boolean isReady() {
      ...
      if (isInputPipeConnected(3) && !isInputPipeConnected(4)) {
         ...
         return (getInputPipeSize(0) > 0 && getInputPipeSize(1) > 0 &&
                    getInputPipeSize(2) > 0 && getInputPipeSize(3) > 0);

      } else if (!isInputPipeConnected(3) && isInputPipeConnected(4)) {

         return (getInputPipeSize(0) > 0 && getInputPipeSize(1) > 0 &&
                    getInputPipeSize(2) > 0 && getInputPipeSize(4) > 0);
   ...
   /**
    * this property is the maximum acceptable rule size.  Must be positive
    * integer.
    *
    * @param  yy new max rule size
    *
    * @throws PropertyVetoException when yy is not a positive integer
    */
   public void setMaxRuleSize(int yy) throws PropertyVetoException {

      if (yy <= 0) {

         throw new PropertyVetoException("Rule size must be a positive integer",
                                         null);
   ...
   /**
    * this property is the min acceptable support score.  Must be at least zero
    * and less than 100
    *
    * @param  i new support
    *
    * @throws PropertyVetoException when i is less than zero or greater than 100
    */
