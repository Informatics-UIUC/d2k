 ...
 * @version 1.0
 */
public class SQLHTree extends ComputeModule implements java.io.Serializable {

   //~ Static fields/initializers **********************************************

   /** Use serialVersionUID for interoperability. */
   static private final long serialVersionUID = 8645396428267611991L;
   ...
   private static int MAXTABLENAME = 30;
         ...
         while (tableSet.next()) {
            ...
            for (int colIdx = 0; colIdx < columnList.size(); colIdx++) {
               ...
               if (!aColumn.isBinned) {
                  ...
                  // ending index in baseHeadTbl for this column.
                  itemIdx =
   ...
   protected void computeItemSet(int headIdx, int hTreeLevel,
      ...
      if (hTreeLevel > 1 && localHeadTblsIdx < (ruleSize - 2)) {
         ...
         for (int colIdx = 1; colIdx < (hTreeLevel); colIdx++) {
            ...
            // this level should stop. The prefixList should be ready for
            // recursion of one level up.
            for (
      ...
      try {
         ...
         if (foundTable(cubeTableName)) {
            ...
            while (tempObj.toString().toUpperCase().indexOf("CUBE") < 0) {
               JOptionPane.showMessageDialog(null,
                                             "The cube table must contain the word 'CUBE'.",
                                             "Error",
                                             JOptionPane.ERROR_MESSAGE);
               tempObj =
                  JOptionPane.showInputDialog(inputBoard,
         ...
         // count column Convert all numeric data types to character data type
         ...
         while (tableSet.next()) {
            ...
            if (isChosenColumn(cname)) {
               ...
               // A binned Categorical column may have a value that is larger
               // than original data.
               ...
               if (ctype.indexOf("CHAR") >= 0) {
                  ...
                  // determine the column size using the max length of binned
                  // value.
         ...
         // aggregated record. There is no value for all columns except the
         // column cnt which is the total number of rows in the data set.
         sb =
   ...
   protected boolean isChosenColumn(String columnName) {
      ...
      for (int colIdx = 0; colIdx < columnList.size(); colIdx++) {
         ...
         if (
             ((Column) columnList.get(colIdx)).columnName.toUpperCase().equals(columnName)) {
            match = true;
   ...
   protected void saveLocalHeadTbl(int localTblsIdx) {
      ...
      for (int localIdx = 0; localIdx < localHeads.size(); localIdx++) {
         ...
         if (aLocalHead.valueCnt >= cutOff) {
            ...
            queryStr =
               insertStr1 + ", " + newColName + ", set_size, cnt) " +
               insertStr2 + ", '" +
               aLocalHead.value + "', " +
   ...
   protected int updLinks(int hdIdx, int pIdx) {
      ...
      // If the index of first-node >= 0, then follow the index of first-node to
      // hTree. Check the index of parent-node for every node following the
      // chain of same-value-node. If parent is same, update count in hTree. If
      // no same parent found, create a new record in hTree.
   ...
   public void doit() throws Exception {
      ...
      if (totalRow > 0 && rightInput) {
         ...
         if (setColumnList()) {
            ...
            // selected column - 1, otherwise, ruleSize = maxRuleSize
