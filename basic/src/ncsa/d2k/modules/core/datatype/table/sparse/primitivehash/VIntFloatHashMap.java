...
//import ncsa.d2k.modules.projects.vered.sparse.column.SparseFloatColumn;
...
public class VIntFloatHashMap extends TIntFloatHashMap implements VHashMap{
    ...
    public static final float NOT_EXIST = SparseFloatColumn.NOT_EXIST;
/**
 ...
 /**
   ...
   */
   ...
   //creating a map between the old order and the new order.
     ...
     */
   private int getNewKey(float currVal, float[] values, int key, boolean[] ocuupiedIndices){
    int retVal = -1;
    ...
    public float remove(int key) {
        float prev = SparseFloatColumn.NOT_EXIST;
        int index = index(key);
        if (index >= 0) {
            prev = _values[index];
