...
public class SparseMutableTable extends SparseTable
    ...
    protected void insertColumn (AbstractSparseColumn newColumn, int position) {
        ...
        for (int i = 0; i < rowNumbers.length; i++) {
            addCol2Row(position, rowNumbers[i]);            /*      VIntHashSet tempSet = (VIntHashSet) rows.get(rowNumbers[i]);
    ...
    public void removeColumn (int position) {
        ...
        if (col != null) {
            ...
            if (redirectionKey != -1)
                columnRef.remove(redirectionKey);
            else {
                System.out.println("removeColumn: Incomplete removal of column # "
                        + position);
                System.out.println("Could not remove value " + position + " from indirection map.");
            }
    ...
    protected void insertRow (double[] newEntry, int rowIndex, int[] validColumns) {
        ...
        if (validColumns.length > 0) {
            numColumns = validColumns[validColumns.length - 1] + 1;            /* if the new row has less entries, we still have to shift down
