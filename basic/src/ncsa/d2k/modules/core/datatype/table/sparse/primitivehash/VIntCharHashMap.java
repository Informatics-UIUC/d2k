...
//import ncsa.d2k.modules.projects.vered.sparse.column.SparseCharColumn;
 ...
 */
  ...
  */
public class VIntCharHashMap extends TIntHash implements Serializable, VHashMap{

    public static final char NOT_EXIST = SparseCharColumn.NOT_EXIST;
    ...
    public char remove(int key) {
        char prev = SparseCharColumn.NOT_EXIST;
        int index = index(key);
        if (index >= 0) {
            prev = _values[index];
    ...
    private static final class EqProcedure implements VIntCharProcedure {
 ...
 /**
   ...
   */
   private VIntIntHashMap getSortedOrder(int[] validKeys, char[] values){
     ...
     */
   private int getNewKey(char currVal, char[] values, int key, boolean[] ocuupiedIndices){
    int retVal = -1;
