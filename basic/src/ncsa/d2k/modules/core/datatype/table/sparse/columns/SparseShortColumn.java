 ...
 * @version 1.0
 */
public class SparseShortColumn extends AbstractSparseColumn implements NumericColumn {
     ...
     */
    public static int getNewRow (short currVal, short[] values, int row, boolean[] ocuupiedIndices) {
        int retVal = -1;
        ...
        for (int i = row + 1; retVal < 0 && i < values.length && values[i] ==
                currVal; i++) {
            if (!ocuupiedIndices[i]) {
                retVal = i;
            }
    ...
    public Column getSubset (int pos, int len) {
        ...
        return  subCol;        /*       //the map to hold the data of the new column
