...
//import ncsa.d2k.modules.projects.vered.sparse.column.SparseByteColumn;
...
public class VIntByteHashMap extends TIntHash implements Serializable, VHashMap{


    public static final byte NOT_EXIST = SparseByteColumn.NOT_EXIST;
    ...
    public byte get(int key) {
        int index = index(key);
        return index < 0 ? NOT_EXIST : _values[index];
    }
    ...
    public byte remove(int key) {
        byte prev = SparseByteColumn.NOT_EXIST;
        int index = index(key);
        if (index >= 0) {
            prev = _values[index];
    ...
    private static final class EqProcedure implements VIntByteProcedure {
...
/*        VSerializationProcedure writeProcedure = new VSerializationProcedure(stream);
   ...
   */
     ...
     */
   private static int getNewKey(byte currVal, byte[] values, int key, boolean[] ocuupiedIndices){
    int retVal = -1;
    ...
    for(int i=key+1; retVal < 0 && i<values.length && values[i] == currVal; i++)
      if(!ocuupiedIndices[i])
	retVal = i;
