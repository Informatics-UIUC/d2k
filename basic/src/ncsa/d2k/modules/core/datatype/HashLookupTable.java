...
public class HashLookupTable implements Serializable {
    ...
    * factor for the root node based on the Java HashMap default
    ...
    *
    * @return <code>true</code> if and only if the table contains a mapping for
    *         the specified keys.
    */
    ...
    * <code>null</code> if the table contains no such mapping. A return value of
    * <code>null</code> does not necessarily indicate that the table contains no
    * mapping for the keys; it's also possible that the map explicitly maps the
    * key to <code>null</code>. (The <code>containsKeys</code> method can be
    ...
    *         null</code> if no such mapping exists.
    */
   ...
   /**
    * Returns a HashMap containing as values all objects on a given level of the
    * table. The keys are the Integer order of placement into the table, indexed
    * from zero.
    *
    ...
    */
   public HashMap getMerged(int level) { return (HashMap) all_maps.get(level); }
    ...
    *
    * @param  keys  Ordered keys with which the specified value is to be
    *               associated.
    ...
    *         previously associated <code>null</code> with the specified set of
    *         keys. (The <code>containsKeys</code> method can be used to
   ...
   public Object put(Object[] keys, Object value) {
      ...
      for (int i = 0; i < keys.length; i++) {
         ...
         if (!((HashMap) all_maps.get(i)).containsValue(keys[i])) {
            ((HashMap) all_maps.get(i)).put(new Integer(((HashMap) all_maps.get(i))
                                                           .size()), keys[i]);
         }
      }
