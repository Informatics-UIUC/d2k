 ...
 */
public final class SVMModel_MultiClass extends PredictionModelModule
    ...
    */
   SVMModel_MultiClass(svm_model[] _models, ExampleTable vt) {
      super(vt);
      this.models = _models;
      setName("SVM Multi Class Prediction Model");
   }
   ...
   protected void makePredictions(PredictionTable pt) {
      ...
      for (int i = 0; i < pt.getNumRows(); i++) {
         ...
         System.out.println(getAlias() + ": record # " + i);
         for(int pred=0; pred<models.length; pred++){
           int prediction = (int)svm.svm_predict_probability(models[pred], x, probs);
           int probIndex = 1;
           if(prediction == -1)
             probIndex = 0;
           pt.setDoublePrediction(  prediction*probs[probIndex], i, pred);


//           v = svm.svm_predict(models[pred], x);
           //pt.setDoublePrediction(  v, i, pred);
           System.out.print("prediction for class " + pred + " = " + prediction);
           System.out.println(" -- probability = " + probs[probIndex]);
         }
   ...
   public String[] getOutputTypes() {
      String[] out =
      {
         ...
         "ncsa.d2k.modules.core.prediction.svm.SVMModel_MultiClass"
      };
