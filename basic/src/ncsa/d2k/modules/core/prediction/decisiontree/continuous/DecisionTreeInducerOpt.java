...
public class DecisionTreeInducerOpt extends FunctionInducerOpt {
   ...
   public String getModuleInfo() {
      ...
      s += "The module implements a decision tree learning algorithm for ";
      s += "continuous attributes. ";
      s += "For inducing functions within a node, one of two different ";
      s += "learning algorithms can be used. ";
      s += "<ul>";
      s += "<li><i>Use the mean averaging for models</i>:  Simple averaging ";
      s += "of output values.  </li>";
      s += "<li><i>Use multiple regression for models</i>:  Using the best ";
      s += "1-d linear function using a single input attribute.  </li>";
      s += "</ul>";
      s += "It allows for multiple decomposition strategies to be used ";
      s += "simultaneously. ";
      s += "<ul>";
      s += "<li><i>Generate splits only at 1/2</i>:  Generate splits only at ";
      s += "0.5.  Works well when inputs are scaled from 0.0 to 1.0.</li>";
      s += "<li><i>Generate mean splits</i>:  Generate splits at the mean ";
      s += "input attribute value within the node.  </li>";
      s += "<li><i>Generate midpoint splits</i>:  Generate splits at the ";
      s += "midpoint between the min and max input attribute value within ";
      s += "the node.  </li>";
      s += "<li><i>Generate median splits</i>:  Generate splits at the median ";
      s += "input attribute value within the node which requires n log n ";
      s += "sorting.  </li> ";
      s += "</ul>";
      s += "Determining whether to split a node is controlled by the following ";
      s += "parameters: ";
      s += "<i>Minimum examples per leaf</i> and <i>Minimum split error ";
      s += "reduction</i>.  ";
      s += "A split is not considered if it results in a node with less than ";
      s += "<i>Minimum examples per leaf</i> examples in it.  ";
      s += "A split is not considered if the resubstitution based error ";
      s += "weighted by population ";
      ...
      s += "<p>Scalability: This module can efficiently process a data set ";
      s += "that can be stored in memory.  The ultimate limit is how much ";
      s += "virtual memory java can access. </p> ";
