 ...
 *
 * <p>Description: Build a decision tree from a database table. If the data set
 * can be fit into the memory, the whole data set will be retrieved and reside
 * in memory, otherwise, the data set will be partitioned at each tree node</p>
 *
 * <p>This implementation is based on RainForest and C4.5 algorithm with
 * following modifications.</p>
 *
 * <p>1. a special class AvcSet (Attribute-Value-Class Set) keeps the summarized
 * information, which reduces the number of data scans and the requirement of
 * memory.</P>
 *
 * <p>2. gain ratio (gain/split-info) is used in categorical columns, but not
 * numeric columns</p>
 *
 * <p>3. minimum ratio (number_record_in_leaf / total_number_records) is used
 * instead of minimumRecordPerLeaf</p>
 *
 * <p>4. missing values are skipped in each column computation, but the records
 * are not removed from the data set.</p>
 *
 * <p>5. dominate ratio (number_mostCommonClass / number_secondMostCommonClass)
 * is used to prune trivial nodes</p>
 *
 * <p>6. Numeric columns are allowed to use more than once, but if the
 * dominateRatio between parent node and child node has no improvement, the
 * spliting will be terminated and a leaf will be formed</p>
 *
...
public class NewSQLRainForestOPT extends ReentrantComputeModule {
   ...
   /** constant for emtpy string */
   static String NOTHING = "";

   /** constant for tab delimiter */
   ...
   /**
    * Creates a new NewSQLRainForestOPT object.
    */
    ...
    * by recursive call buildTree.
    *
    * @param  path      the spliting values from root to the current node
    * @param  availCols the candidate columns to use
    * @param  aNodeInfo an object of NodeInfo that includes: avcSets, data,
    *                   classTallies, and uniqValues
    * @param  examples  the index list of example set (this param only used in
    *                   in-memory mode)
    *
    * @return a tree node
    */
   ...
   int[] examples) {
      ...
      if (total < minimumRecordsPerLeaf) {
         ...
         // System.out.println("create a leaf for data set <
         // minimumRecordsPerleaf");
         ...
         root =
            new CategoricalDecisionForestNode(mostCommon, classValues.length);
      ...
      // if all avcSets are in a same class, give the root this label -- this
      // node is a leaf
         ...
         if (best != null) {
            ...
            if (!meta.isColumnScalar(col)) {
               ...
               // for each possible value v of this attribute add a new branch
               // below the root,

               // corresponding to A = v

               root =
                  new CategoricalDecisionForestNode(meta.getColumnLabel(col)
                                                        .toString(),
                                                    classValues.length);
               ...
               // Since avcs contains the pairs of column value with class
               // label, e.g.

               // "Race=F,Yes", "Race=F,No", we need to get distinct values in
               // the column.
               ...
               for (int idx = 0; idx < distinctValues.size(); idx++) {
                  ...
                  String newAttrValue =
                     meta.getColumnLabel(col) + "='" + newValue + "'";
                  ...
                  newCols =
                     narrowAttributes(meta.getColumnLabel(col), availCols);
                  ...
                  if (newCols.length > 0) {
                     ...
                     int subTot =
                        subTotal(avcs, meta.getColumnLabel(col), newValue);
                     ...
                     if (examples == null && subTot >= modeThreshold) {
                        ...
                        newUniqValues =
                           getUniqValues(dbTable, newPath, newCols);

                        newNodeInfo =
                           extractDataFromDB(dbTable, classColName, newPath,
                                             newCols, uniqValues);
                     ...
                     // Switch to in-memory mode from in-database mode
                        ...
                        newNodeInfo =
                           createDataTable(dbTable, meta, whereClause, newPath,
                                           subTot);
                        ...
                        for (
                             int rowIdx = 0;
                                rowIdx < newNodeInfo.data.getNumRows();
                                rowIdx++) {
                        ...
                        branchExam =
                           narrowCategoricalExamples(data, col, newValue,
                                                     examples);

                        newUniqValues =
                           getUniqValues(data, branchExam, newCols);

                        newNodeInfo =
                           partitionDataSet(data, branchExam, newCols,
                                            newUniqValues);
                     ...
                     if (
                         getTotalTallies(newNodeInfo.classTallies) >=
                            minimumRecordsPerLeaf) {

                        DecisionForestNode tmpRoot =
                           buildTree(newPath, newCols, newNodeInfo, branchExam);
                        ...
                        // System.out.println("create a leaf because the
                        // tallies < minimumRecordsPerLeaf.");

                        String mostCommon =
                           mostCommonOutputValue(newNodeInfo.classTallies);

                        CategoricalDecisionForestNode tmpRoot =
                           new CategoricalDecisionForestNode(mostCommon,
                                                             classValues.length);
               ...
               // if branchCount == 0, that indicates all branches have been
               // pruned, create a leaf to attach to the root

               if (branchCount == 0) {

                  // System.out.println("All branches have been pruned, create
                  // a leaf.");

                  String mostCommon =
                     mostCommonOutputValue(aNodeInfo.classTallies);
               ...
               root =
                  new NumericDecisionForestNode(meta.getColumnLabel(col)
                                                    .toString(),
                                                classValues.length);
               ...
               String leftLabel =
                  meta.getColumnLabel(col) + " < " + nf.format(newValue);
               ...
               int subTot =
                  subTotal(avcs, meta.getColumnLabel(col), "<" +
               ...
               // Switch to in-memory mode from in-database mode
                  ...
                  branchExam =
                     narrowNumericExamples(data, col, best.splitValue, examples,
                                           false);

                  newUniqValues = getUniqValues(data, branchExam, availCols);

                  newNodeInfo =
                     partitionDataSet(data, branchExam, availCols,
                                      newUniqValues);
               ...
               if (
                  ...
                  // If the numeric attribute has been used once, compare the
                  // dominateRatio with

                  // parent nodes. Only generate a node if the dominateRatio
                  // improves.

                  if (repeats(meta.getColumnLabel(col), newPath) >= 2) {

                     if (
                         isRatioUp(aNodeInfo.classTallies,
                                      newNodeInfo.classTallies)) {

                        left =
                           buildTree(newPath, availCols, newNodeInfo,
                                     branchExam);

                     } else { // create a leaf because dominate ratio is not up

                        // System.out.println("create a left leaf because the
                        // dominate ratio is not up.");
                     ...
                     left =
                        buildTree(newPath, availCols, newNodeInfo, branchExam);

                  }

               } else { // total tallies are < minimumRecordsPerLeaf

                  // System.out.println("create a left leaf because tallies are
                  // < minimumRecordsPerLeaf");
               ...
               if (
                  ...
                  if (repeats(meta.getColumnLabel(col), newPath) >= 2) {
                     ...
                     if (
                        ...
                        // System.out.println("create a right leaf because the
                        // dominate ratio is not up.");
                  ...
                  // System.out.println("create a right leaf because tallies
                  // are < minimumRecordsPerLeaf");
               ...
               if (left != null && right != null) {
                  ...
                  root.addBranches(newValue, leftLabel, left, rightLabel,
                                   right);
                  ...
                  // System.out.println("create a leaf because there is no
                  // branch to add");
         ...
         // a new node with the most common output
            ...
            // System.out.println("make a leaf because there is no best
            // column");
   ...
   private double categoricalGain(ArrayList aPath, String column,
                                  NodeInfo aNodeInfo) {
      ...
      for (int idx = 0; idx < distinctValues.size(); idx++) {
         ...
         int[] childTallies =
            getClassTallies(column, distinctValues.get(idx).toString(), avcs);
      ...
      // since sInfo could be 0 if all examples have a same value in this
      // column, add 0.000001 to it
   ...
   /**
    * This method scans the database once and does the following tasks. 1.
    * insert data into an in-memory table 2. build the ArrayList uniqValues 3.
    * get the String[] classValues 4. compute the int[] classTallies 5. build
    * the avcSets from the in-memory table
    *
    * @param  dbTable     the database table to read
    * @param  meta        the meta information to use
    * @param  whereClause Description of parameter whereClause.
   ...
   private NodeInfo createDataTable(String dbTable, ExampleTable meta,
                                    String whereClause,
   ArrayList path, int total) {
      ...
      try {
         ...
         if (whereClause != null && whereClause.length() > 0) {
            ...
            dataQry =
               dataQry + " from " + dbTable + " where (" + whereClause +
               ") and " +
            ...
            dataQry =
               dataQry + " from " + dbTable + " where " + classColName +
         ...
         while (dataSet.next()) {
            ...
            for (int colIdx = 0; colIdx < data.getNumColumns(); colIdx++) {
               ...
               // build ArrayList uniqValues and insert data in to the data
               // table
               ...
               if (data.isColumnScalar(colIdx)) {
                  ...
                  if (
                     ...
                     if (
                         !inList(Double.toString(attrValue),
                                    (ArrayList) uniqValues[colIdx])) {
         ...
         for (int row = 0; row < data.getNumRows(); row++) {
            ...
            for (int col = 0; col < data.getNumColumns(); col++) {
               ...
               if (col != outputFeatures[0] && data.isColumnScalar(col)) {
                  ...
                  avcSets =
                     updAvcSets(avcSets, attrName, attrValue, classLabel,
                                uniqValues);

               } else if (
                          col != outputFeatures[0] &&
                             !data.isColumnScalar(col)) {
         ...
         NodeInfo aNodeInfo =
            new NodeInfo(avcSets, classTallies, data, uniqValues);
   ...
   private NodeInfo extractDataFromDB(String dbTable, String classColName,
                                      ArrayList path,
   String[] availCols, ArrayList[] uniqValues) {
      ...
      if (availCols.length == 0) {
         ...
         NodeInfo aNodeInfo =
            new NodeInfo(avcSets, classTallies, null, uniqValues);
         ...
         while (dataSet.next()) {
            ...
            for (int colIdx = 0; colIdx < availCols.length; colIdx++) {
               ...
               if (!isScalar(availCols[colIdx])) {
                  ...
                  // dataSet starts from index 1 and the first column is
                  // classLabel, therefore need colIdx+2
                  ...
                  if (
                     ...
                     avcSets =
                        updOneAvc(avcSets, attrName, attrValue, classLabel);
   ...
   private int[] getClassTallies(String column, String val, ArrayList[] avcs) {
      ...
      if (!meta.isColumnScalar(col)) {
         ...
         if (val.indexOf(">=") >= 0) {
            ...
            double tmpValue =
               Double.parseDouble(val.substring(val.indexOf(">=") + 2,
                                                val.length()));
   ...
   private String[] getClassValues(String classColName) {
      ...
      try {
         ...
         if (whereClause != null && whereClause.length() > 0) {
            ...
            classQry =
               new String("select distinct " + classColName + " from " +
                          dbTable +

                  " where (" + whereClause + ") and " + classColName +
                          " is not null");
    ...
    * @param  aNodeInfo object contains classTallies, avcSets, data table,
    *                   unique value list.
    *
    ...
    * @param  path      an ArrayList that keeps all branch labels from root to
    *                   the current node
   ...
   private ArrayList[] getUniqValues(String dbTable, ArrayList path,
                                     String[] availCols) {
      ...
      try {
         ...
         for (int colIdx = 0; colIdx < meta.getNumColumns(); colIdx++) {
            ...
            if (whereClause != null && whereClause.length() > 0) {
               ...
               valueQry =
                  valueQry + dbTable + " where (" + whereClause + ") and " +
                  colName + " is not null";

            } else {

               valueQry =
                  valueQry + dbTable + " where " + colName + " is not null";
            }
            ...
            if (!meta.isColumnScalar(colIdx)) { // non-numeric column
               ...
               // if the uniq value <= binNumber, use all values as the split
               // values

               if (aUniqCol.size() <= binNumber) {

                  // the split value will be the halfway point between two
                  // values
                  ...
                  for (
                       int listIdx = 0;
                          listIdx < aUniqCol.size() - 1;
                          listIdx++) {

                     double tmpVal =
                        (Double.parseDouble(aUniqCol.get(listIdx).toString()) +
                           Double.parseDouble(aUniqCol.get(listIdx + 1)
                                                      .toString())) / 2;
               ...
               // if the uniq value > binNumber, calculate the split points
                  ...
                  double max =
                     Double.parseDouble(aUniqCol.get(aUniqCol.size() - 1)
                                                .toString());

                  // rebuild the ArrayList to keep spliting values for
   ...
   private ArrayList[] getUniqValues(MutableTableImpl data, int[] examples,
                                     String[] availCols) {
         ...
         if (!inList(meta.getColumnLabel(colIdx), availCols)) {
            ...
            if (aUniqCol.size() <= binNumber) {
               ...
               for (int listIdx = 0; listIdx < aUniqCol.size() - 1; listIdx++) {
                  ...
                  double tmpVal =
                     (Double.parseDouble(aUniqCol.get(listIdx).toString()) +
                        Double.parseDouble(aUniqCol.get(listIdx + 1).toString())) /
                        2;
            ...
            // if the uniq value > binNumber, calculate the split points
               ...
               // rebuild the ArrayList to keep spliting values for specified
               // number of bins
    ...
    * parent node and the current node, return true if the difference >
    * improvementRatio.
    *
   ...
   /**
    * find whether all avcSets are in a same class Modified the original
    * algorithm as the following: If (%mostCommonVal) / (%secondMostCommonVal) >
    * dominateRatio, return true. This will prune some trivial node.
    *
   ...
   private boolean isSingleClass(int[] classTallies) {
      ...
      if ((total - mostCommonVal) == 0) {
         ...
         return true;
                 ...
                 ((double) mostCommonVal / (double) secondMostCommonVal) >
                    dominateRatio) {
   ...
   private double numericAttributeInfo(double aSplitValue, String column,
      ...
      if (lessThanTot == 0 && greaterThanTot == 0) {
         ...
         double retval =
            (lessThanTot / (double) (lessThanTot + greaterThanTot)) * linfo +
   ...
   private EntrSplit numericGain(ArrayList aPath, String column,
      ...
      for (int rowIdx = 0; rowIdx < aColumn.size(); rowIdx++) {
         ...
         double aSplitValue =
            Double.parseDouble(aColumn.get(rowIdx).toString());

         double gain =
            baseGain - numericAttributeInfo(aSplitValue, column, avcs);
         ...
         for (int colIdx = 0; colIdx < data.getNumColumns(); colIdx++) {
            ...
            if (colIdx == outputFeatures[0]) {
               ...
               for (
                  ...
                  // System.out.println("in partition, tmpString is " +
                  // tmpString + ", classValues is " + classValues[valueIdx]);

                  if (tmpString.equals(classValues[valueIdx])) {

                     classLabel = tmpString;

                     // System.out.println("in partition, classLabel set to " +
                     // classLabel);
   ...
   } // end method partitionDataSet


   /**
    * print the contents of an array list
    *
    * @param al an array list
    */
   ...
   /**
    * print the contents of avc sets
    *
    * @param avcs avc sets
    */
    ...
    * values less than binNumber, use the mid points of each two values as the
    * split points. Otherwise, calculate the split point for each bin.
    *
         ...
         for (int setIdx = 0; setIdx < avcs[colIdx].size(); setIdx++) {
            ...
            if (aSet.attrName.equals(column)) {
               ...
               if (!map.containsKey(aSet.lowValue)) {
                  ...
                  tallies.set(ii.intValue(), tallies.get(ii.intValue()) +
                              aSet.count);
   ...
   private int subTotal(ArrayList[] avcs, String columnLabel,
      ...
      if (!meta.isColumnScalar(colIdx)) {
         ...
         for (int setIdx = 0; setIdx < avcs[colIdx].size(); setIdx++) {
            ...
            if (
                aSet.attrName.equals(columnLabel) &&
         ...
         if (columnValue.indexOf(">=") >= 0) {
            ...
            double tmpValue =
               Double.parseDouble(columnValue.substring(columnValue.indexOf(">=") +
                                                        2,
                                                        columnValue.length()));
    ...
    * @param  uniqValues the unique values in the data set. This will be the
    *                    split values for numeric data.
    *
   ...
   String classLabel, ArrayList[] uniqValues) {
      ...
      for (int rowIdx = 0; rowIdx < uniqValue.size(); rowIdx++) {
         ...
         if (attrValue < currSplit && attrValue >= prevSplit) {
            ...
            avcSets =
               updOneAvc(avcSets, attrName, newLowValue, newHighValue,
                         classLabel);
      ...
      if (!done && attrValue >= prevSplit && attrValue < Double.MAX_VALUE) {
         ...
         avcSets =
            updOneAvc(avcSets, attrName, newLowValue, newHighValue, classLabel);

      }

      return avcSets;

   } // end method updAvcSets
   ...
   /**
    * create a new array list, first copy the original array list then add a new
    * value to the new array list.
    *
   ...
   protected ArrayList sortUniqValues(ArrayList al) {
      ...
      for (int i = 0; i < al.size(); i++) {
         ...
         for (int j = al.size() - 1; j > i; j--) {
            ...
            if (
                Double.parseDouble(al.get(j - 1).toString()) >
   ...
   public void doit() throws Exception {
      ...
      if (meta.getOutputFeatures().length == 0) { // no class label is defined
         ...
         if (totalRow > 0 && totalRow < modeThreshold) {
            ...
            NodeInfo aNodeInfo =
               createDataTable(dbTable, meta, whereClause, splitValues,
            ...
            for (
                 int rowIdx = 0;
                    rowIdx < aNodeInfo.data.getNumRows();
                    rowIdx++) {
            ...
            DecisionForestNode rootNode =
               buildTree(splitValues, availableCols,


               aNodeInfo, exampleSet);

            DecisionForestModel mdl =
               new DecisionForestModel(rootNode, meta,
            ...
            NodeInfo aNodeInfo =
               extractDataFromDB(dbTable, classColName,
    ...
    * @return bin number
    */
   public double getBinNumber() { return binNumber; }


    /**
     * get the dominate ratio.
     *
     * @return dominate ratio
     */
   public double getDominateRatio() { return dominateRatio; }
    ...
    public String getInputInfo(int i) {
        ...
        switch (i) {
            ...
            case 2:
                return "The table that contains meta information.";

            case 3:
                return "The query condition for filtering data.";

            case 4:
                return "The parameters used in this module.";
     ...
     *
     * @return minimum ratio per leaf
     */
   public double getMinimumRatioPerLeaf() { return minimumRatioPerLeaf; }


    /**
     * get the mode threshold.
     *
     * @return mode threshold
     */
   public double getModeThreshold() { return modeThreshold; }
    ...
    public String getModuleInfo() {
        ...
        s +=
                "The module supports two different modes to build a decision tree. If the ";
        ...
        s +=
                "This module is based on RainForest and C4.5 algorithms with the following features. </p>";

        s +=
                "<ul><li>a special class AvcSet (Attribute-Value-Class Set) keeps the ";
        ...
        s +=
                "<li>minimum-ratio-per-leaf (number-record-in-leaf / total-number-records) is ";
        ...
        s +=
                "<li>dominate ratio (number_mostCommonClass / number_secondMostCommonClass) ";

        s += "is used to prune trivial nodes. </li> ";

        s +=
                "<li>Numeric columns are allowed to be chosen more than once. However, if the dominateRatio ";

        s +=
                "between parent node and child node shows no improvement, the spliting will ";
        ...
        s +=
                "<ul><li>C4.5: Programs for Machine Learning by J. Ross Quinlan </li>";
        ...
        s +=
                "nominal outputs. We currently only support Oracle, SQLServer, DB2 and MySql databases. ";

        s +=
                "There are two implementations on RainForest. <i>NewSQLRainForestOPT</i> does ";

        s +=
                "not do parameter optimization, but <i>NewSQLRainForestOPTOPT<i> does.</p>";
    ...
    public String getOutputInfo(int i) {
        ...
        switch (i) {
            ...
            case 0:
                return "The generated decision tree model.";
    ...
    public PropertyDescription[] getPropertiesDescriptions() {
        ...
        pds[0] =
                new PropertyDescription("minimumRatioPerLeaf", MIN_RATIO,
        ...
        pds[1] =
                new PropertyDescription("modeThreshold", MODE_THRESHOLD,
        ...
        pds[2] =
                new PropertyDescription("binNumber", BIN_NUMBER,
        ...
        pds[3] =
                new PropertyDescription("dominateRatio", DOMINATE_RATIO,
   ...
   public void setBinNumber(double i) throws PropertyVetoException {
    ...
    /**
     * set the dominate ratio.
     *
     * @param num the new dominate ratio
     * @throws java.beans.PropertyVetoException
     *          when num is less than zero
     */
   public void setDominateRatio(double num) throws PropertyVetoException {
     ...
     *
     * @param num the new minimum ratio per leaf
     * @throws java.beans.PropertyVetoException
     *          when ratio is less than zero or greater than
     *          one
     */
   public void setMinimumRatioPerLeaf(double num) throws PropertyVetoException {
    ...
    /**
     * set the mode threshold.
     *
     * @param num the new mode threshold
     * @throws java.beans.PropertyVetoException
     *          when num is less than zero
     */
