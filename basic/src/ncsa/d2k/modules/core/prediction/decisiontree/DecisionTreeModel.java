...
// private int trainingSetSize;
   ...
   /** Description of field uniqueInputs. */
 ...
 * }*/
   ...
   /**
    * Make a prediction for an example.
    *
    * @param example          table of examples
    * @param row              example index
    * @param predictedOutputs Description of parameter predictedOutputs.
    */
   protected void makePrediction(ExampleTable example, int row,
                                 double[] predictedOutputs) { }


/*      public PredictionTable predict(ExampleTable val) {
 *              table = (ExampleTable)val;             numExamples =
 * table.getNumRows();
 *
 *           PredictionTable pt = null;             if (table instanceof
 * PredictionTable)                     pt = (PredictionTable) table;
 *  else                     pt = table.toPredictionTable();
 *
 *           int[] outputs = pt.getOutputFeatures();             int[] preds =
 * pt.getPredictionSet();
 *
 *           if(preds.length == 0) {                     //StringColumn sc = new
 * StringColumn(pt.getNumRows());                     String [] predic = new
 * String[pt.getNumRows()];                     pt.addPredictionColumn(predic,
 * "Predictions");                     preds = pt.getPredictionSet();
 *  }
 *
 *           for(int i = 0; i < pt.getNumRows(); i++) {
 * String pred = (String)root.evaluate(pt, i);
 * pt.setStringPrediction(pred, i, 0);             }             //uniqueOutputs
 * = uniqueValues(pt, outputs[0]);             return pt;     }*/
 ...
 *
 *   public String[] getInputColumnLabels() {             return
 * inputColumnNames;     }
 *
 *   public String[] getOutputColumnLabels() {             return
 * outputColumnNames;     }
 *
 *   public int[] getInputFeatureIndicies() {             return inputFeatures;
 *    }
 *
 *   public int[] getOutputFeatureIndices() {             return outputFeatures;
 *     }
 *
 *   public String[] getInputFeatureTypes() {             String[] retVal = new
 * String[inputIsScalar.length];             for(int i = 0; i <
 * inputIsScalar.length; i++) {                 if(inputIsScalar[i])
 *         retVal[i] = "Scalar";                 else
 * retVal[i] = "Nominal";             }             return retVal;
 * //return inputTypes;     }
 *
 *   public String[] getOutputFeatureTypes() {             String[] retVal = new
 * String[outputIsScalar.length];             for(int i = 0; i <
 * outputIsScalar.length; i++) {                 if(outputIsScalar[i])
 *           retVal[i] = "Scalar";                 else
 * retVal[i] = "Nominal";             }             return retVal;     }*/
