...
public class C45TreeBuilderOPT extends ReentrantComputeModule {
         ...
         if (val < splitVal) {
            ...
            // found a new one..
               ...
               greaterThanIndexMap.put(out,
                                       new Integer(greaterThanIndexMap.size()));
    ...
    */
   protected DecisionTreeNode buildTree(int[] examples, int[] attributes)
      ...
      if (best != null) {
         ...
         if (!table.isColumnScalar(col)) {
            ...
            try {
               ...
               root =
                  new CategoricalDecisionTreeNode(table.getColumnLabel(col));
               ...
               for (int i = 0; i < branchVals.length; i++) {
                  ...
                  if (
                      (((double) branchExam.length) / (double) numExamples) >
                         getMinimumRatioPerLeaf() &&
                         branchAttr.length != 0) {
...
/*  private int minimumRecordsPerLeaf = 2; */
    ...
    */
   protected double getMinimumRatioPerLeaf() { return minimumRatioPerLeaf; }
 ...
 */
      ...
      // pp.getValue(C45ParamSpaceGenerator.MIN_RATIO);
      setMinimumRatioPerLeaf(pp.getValue(C45ParamSpaceGenerator.MIN_RATIO));
      // if(minimumRecordsPerLeaf < 1)
      // throw new Exception(getAlias()+": Must be at least one record per
      // leaf!");
    ...
    */
   public String getModuleName() { return "Optimized C4.5 Tree Builder"; }
