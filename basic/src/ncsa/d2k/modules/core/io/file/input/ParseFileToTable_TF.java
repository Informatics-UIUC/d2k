...
/**
 * Given a FlatFileReader, create a Table initialized with its contents.
 */
public class ParseFileToTable_TF extends InputModule {
    ...
    public String getModuleInfo() {
        ...
        sb.append("initialized with the contents of a flat file from disk.  The type of table is determined");
        sb.append(" by the TableFactory used.");
        sb.append("</p>");
        return sb.toString();
    }

    public String getModuleName() {
        return "Parse File To Table TF";
    }

    public void doit() throws Exception {
        FlatFileParser fle = (FlatFileParser)pullInput(0);
        TableFactory fact = (TableFactory)pullInput(1);
        Table t = createTable(fle, fact);
        pushOutput(t, 0);
    }

    public Table createTable(FlatFileParser df, TableFactory factory) {
        int numRows = df.getNumRows();
        int numColumns = df.getNumColumns();

        boolean hasTypes = false;

        MutableTable ti = (MutableTable)factory.createTable();
        ...
        if(!hasTypes) {
           ...
           // System.out.println("no types");
            ...
            for(int i = 0; i < numColumns; i++) {
                ...
                if(isNumeric) {
                    Column dc = factory.createColumn(ColumnTypes.DOUBLE);
