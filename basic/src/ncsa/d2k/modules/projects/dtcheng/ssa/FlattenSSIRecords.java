...
public class FlattenSSIRecords extends InputModule {
  
  int startFileNumber    = 3;
  int numFilesToProcess = 1;
  
  int totalNumFiles      = 14;
  ...
  String matrixDirectory = "c:/data/matrix/ssi";
  ...
  public void setStarFileNumber(int value) {
    this.startFileNumber = value;
  }
  public int getStarFileNumber() {
    return this.startFileNumber;
  }
  
  public void setNumFilesToProcess(int value) {
    this.numFilesToProcess = value;
  }
  public int getNumFilesToProcess() {
    return this.numFilesToProcess;
  }
  ...
  int[]      totalNumFields = new    int[totalNumFiles];
  
  String[][]  fieldNames     = new  String[totalNumFiles][];
  String[][]  fieldValues    = new  String[totalNumFiles][];
  int[][]     fieldSizes     = new     int[totalNumFiles][];
  byte[][][]  fieldBytes     = new    byte[totalNumFiles][][];
  int[]       recordSizes    = new     int[totalNumFiles];
  boolean[][] readField      = new boolean[totalNumFiles][];
  boolean[][] inputField     = new boolean[totalNumFiles][];
  int[][] fieldInputIndex    = new int[totalNumFiles][];
  
  
  
  long[]    byteIndices        = new    long[totalNumFiles];
  int[]     recordIndices      = new     int[totalNumFiles];
  boolean[] EOFs               = new boolean[totalNumFiles];
  int[]     numPersonIDRepeats = new     int[totalNumFiles];
  long[]    masterSums         = new    long[totalNumFiles];
  String[]  lastKeys           = new  String[totalNumFiles];
  
  BufferedInputStream[] bufferedInputStream = new BufferedInputStream[totalNumFiles];
  ...
  public int calculateFileStructures() throws Exception {
    ...
    for (int fileIndex = 0; fileIndex < totalNumFiles; fileIndex++) {
  ...
  public void closeFile(int fileIndex) throws Exception {
    bufferedInputStream[fileIndex].close();
  }
  ...
  public void doit() throws Exception {
    ...
    for (int fileIndex = (startFileNumber - 1); fileIndex < (startFileNumber - 1) + numFilesToProcess; fileIndex++) {
      ...
      System.out.println("initializing external matrix");
      ...
      ByteMatrix byteMatrix = new ByteMatrix(ByteMatrix.dataInGzipObjectFiles, dimensions, bufferSize, gzipBufferSize, matrixDirectory, matrixName, true);
      ...
      for (int recordIndex = 0; recordIndex < spec.NumRecords[fileIndex]; recordIndex++) {
        ...
        if (recordIndex == numRecordsToRead)
          break;
        ...
        for (int fieldIndex = 0; fieldIndex < totalNumFields[fileIndex]; fieldIndex++) {
          ...
          for (int i = 0; i < fieldSizes[fileIndex][fieldIndex]; i++) {
            ...
            byteMatrix.setValue(coordinates, byteBuffer[i]);
          }
