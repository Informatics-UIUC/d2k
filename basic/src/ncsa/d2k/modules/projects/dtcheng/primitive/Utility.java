...
public class Utility {
  ...
  static public int [] randomIntArray(int seed, int numElements) throws Exception {
  ...
  static public int getStringIndex (String key, String [] keyArray) {
    int i = 0;
  ...
  static public String[] parseList(String list, char delimiterChar) {

    return parseList(list, delimiterChar, false);
  }

  static public String[] parseList(String list, char delimiterChar, boolean useQuotes) {
    ...
    if (useQuotes) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (!inQuote && (list.charAt(i) == delimiterChar)) {
          numCommas++;
        }
      }
    } else {
      for (int i = 0; i < length; i++) {
        if (list.charAt(i) == delimiterChar) {
          numCommas++;
        }
      }
    }
