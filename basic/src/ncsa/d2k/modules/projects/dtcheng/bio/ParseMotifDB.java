...
public class ParseMotifDB
  ...
  public void doit() throws Exception {
    ...
    for (int SpeciesIndex = 0; SpeciesIndex < NumSpecies; SpeciesIndex++) {
      ...
      FlatFile NewFlatFile = new FlatFile(FileName, "r", 100000, true);
      NewFlatFile.DelimiterByte = FlatFile.TabByte;

      byte [] buffer = NewFlatFile.Buffer;
      int [] ColumnStarts =  NewFlatFile.ColumnStarts;
      int [] ColumnEnds   =  NewFlatFile.ColumnEnds;

      // skip header
      int NumColumns;
      String LineString;     NewFlatFile.readLine();
      if (false) {
        NumColumns = NewFlatFile.NumColumns;
      ...
      while (!NewFlatFile.EOF) {

        NewFlatFile.readLine();
