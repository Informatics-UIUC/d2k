...
public class NetworkedNNWriter_MNL_SharedMemory extends OutputModule {
	
	public String getModuleName() {
		return "NetworkedNNWriter_MNL_SharedMemory";
	}
	
	public String getModuleInfo() {
		return "This module has been adapted for a shared-memory machine and hence a machine list is unnecessary. "
		+ "<p> This module is a pure brute force, fully MAGIC'ed attempt "
		...
		+ "<p>THIS IS CONFIGURED TO MATCH THE MULTINOMIAL-LOGIT (NO HIDDEN LAYERS) TEMPLATES" +
		" IN ORDER TO DO A NEWTON-RAPHSON SOLVER INSTEAD OF GRADIENT DESCENT";
	}
	...
	public void doit() throws Exception {
		...
		int nThreadsLocalhost = ((Integer)this.pullInput(0)).intValue();
		String BaseDataPathName = (String)this.pullInput(1);
		String InitializationFileName = (String)this.pullInput(2);
		String NewItineraryName = (String)this.pullInput(3);
		String QueenTemplateFileName = (String)this.pullInput(4);
		String WorkerTemplateFileName = (String)this.pullInput(5);
		
		int largeInteger = 10000000; // Beware the MAGIC NUMBER!!! the
		int nMachines = nThreadsLocalhost;
...
//		int destinationNumberSignIndex = -103;
		...
		while (true) {
			 ...
			 */				
				...
				QueenLineContents = doFanIn(NewItineraryWriter, QueenReader, QueenLineContents, nMachines);
			} else if (moduleAlias.equals("FanIn Hessian Collector")){
				QueenLineContents = doFanIn(NewItineraryWriter, QueenReader, QueenLineContents, nMachines);
			} else if (moduleAlias.equals("FanIn TrainingError Collector")){
				QueenLineContents = doFanIn(NewItineraryWriter, QueenReader, QueenLineContents, nMachines);
			} else if (moduleAlias.equals("FanIn StackedGradient Collector")){
				QueenLineContents = doFanIn(NewItineraryWriter, QueenReader, QueenLineContents, nMachines);
...
//			NewItineraryWriter.println("<!-- Begin Worker #" + machineIndex + "-->");
			...
			while (true) {
				...
				// doNNWorkerToCollectors subroutine
				else if (moduleAlias.equals("# MFMToDoubleArray: Hessian")) {
					// handle the module invocation
					...
					NewItineraryWriter.println("        <destination alias=\"FanIn Hessian Collector\" index=\""
							+ (machineIndex + 1) + "\"/>");
				...
				} else if (moduleAlias.equals("# MFMToDoubleArray: StackedGradient")) {
					// handle the module invocation
					numberSignIndex = WorkerLineContents.indexOf("#");
					ylocStart = WorkerLineContents.indexOf("yloc") + 6;
					ylocEnd = WorkerLineContents.indexOf("\"",ylocStart);
					...
					// do the output line manually
					NewItineraryWriter.println("      <output index=\"0\" classname=\"[[D\">");
					NewItineraryWriter.println("        <destination alias=\"FanIn StackedGradient Collector\" index=\""
							+ (machineIndex + 1) + "\"/>");
					NewItineraryWriter.println("      </output>");
					// read down to the end of the module
					while (true) {
...
//		NewItineraryWriter.println("<!-- The machine assignments would go here, but we are using shared memory so everything is local. -->");
