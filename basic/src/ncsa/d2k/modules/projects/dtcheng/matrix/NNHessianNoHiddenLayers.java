	...
	public class NNHessianNoHiddenLayers extends ComputeModule {
	
	  public String getModuleName() {
	    return "NNHessianNoHiddenLayers";
	  }
	  ...
	  public void doit() throws Exception {
	    ...
	    MultiFormatMatrix WeightNumberFromToNeuronTable = (MultiFormatMatrix)this.pullInput(2);
...
//	    long nRelevantOutputs = (long) LayerTable.getValue(LayerTable.getDimensions()[0] - 1, 0);
	    ...
	    long nNeurons = NeuronActivations.getDimensions()[1] - 1; // the last output is irrelevant due to standardization...
...
//	    System.out.println("Got to here: before anything difficult... Hessian is defined...");
		...
		synchronized (System.out) {
	          ...
	          long dim1Size = WeightNumberFromToNeuronTable.getDimensions()[0];
	          for (long d1 = 0; d1 < dim1Size; d1++) {
	            coordinates[0] = d1;
	            long dim2Size = WeightNumberFromToNeuronTable.getDimensions()[1];
	            for (long d2 = 0; d2 < dim2Size; d2++) {
	              coordinates[1] = d2;
	              System.out.println("Wn_ft_N " + "[" + d1 + "][" + d2 + "] = "
	              		+ WeightNumberFromToNeuronTable.getValue(coordinates));
	            }
	          }
		}
		*/
...
//	        System.out.println("Before: WeightRow=" + WeightRow + "; WeightCol=" + WeightCol);
	    	i = (long)WeightNumberFromToNeuronTable.getValue(WeightRow,0);
	    	j = (long)WeightNumberFromToNeuronTable.getValue(WeightRow,1);
	    	k = (long)WeightNumberFromToNeuronTable.getValue(WeightCol,0);
	    	ell = (long)WeightNumberFromToNeuronTable.getValue(WeightCol,1);
...
//	        	Hessian.setValue(WeightCol,WeightRow,HessTemp); // live for symmetry...
	    ...
	    for (long BiasIndex = FirstBias; BiasIndex < nBiases; BiasIndex++){
	    	...
	    	ell = BiasIndex; // here, the biases are "terminating" in the neuron with the same number...
...
//	        	Hessian.setValue(ColumnToStore,WeightRow,HessTemp); // live for symmetry...
	    ...
	    for (long BiasIndexCol = BiasIndexRow; BiasIndexCol < nBiases; BiasIndexCol++){
	    	...
	    	j = BiasIndexRow;
	        // k is absent because it would be a "+1 permanently" neuron, but i don't list them, i hardcode them...
	    	ell = BiasIndexCol; // here, the biases are "terminating" in the neuron with the same number...
...
//	        	Hessian.setValue(ColumnToStore,RowToStore,HessTemp); // live for symmetry...
	    ...
	    /*
	     * 
	     * Hessian = zeros(n_weights + n_neurons - n_Xs) + NaN;
	% first, i'll do the weight/weight elements, then we'll do bias/bias, then we'll do weight/bias
	for weight_row = 1:n_weights
	for weight_col = weight_row:n_weights
	        % figure out which neurons are connected by row and column weights under consideration
	    row_from    = Wn_N_ft(weight_row,1); % in Ripley: i
	    row_to      = Wn_N_ft(weight_row,2); % in Ripley: j
	    col_from    = Wn_N_ft(weight_col,1); % in Ripley: k
	    col_to      = Wn_N_ft(weight_col,2); % in Ripley: l
	% what exactly do they connect? we do this by cases...
	            i = row_from;
	            j = row_to;
	            k = col_from;
	            ell = col_to;
	    % case I: both are output units.. that is, both j & ell are output neurons
	    if ((N_to_L(j,1) == n_layers) & (N_to_L(ell,1) == n_layers))
	    Hessian(weight_row,weight_col) = sum(   ...
	                                            ...
	            activation(:,i).*activation(:,k).* ...
	            activation(:,j).*((j == ell) - activation(:,ell)) ...
	                                        );
	%     Hessian(weight_row,weight_col) = sum(   ...
	%                                             ...
	%             activation(:,row_from).*activation(:,col_from).* ...
	...
	% BIASES....
	    ...
	    if ((N_to_L(j,1) == n_layers) & (N_to_L(ell,1) == n_layers))
	    Hessian(bias_row - n_Xs + n_weights,bias_col - n_Xs + n_weights) = sum(   ...
	                                            ...
	            1.*1.* ...
	            activation(:,j).*((j == ell) - activation(:,ell)) ...
	                                        );
