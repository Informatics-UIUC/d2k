...
public class NNActivations2
    extends ComputeModule {

  public String getModuleName() {
    return "NNActivations2";
  }

  public String getModuleInfo() {
    return "Played with to see if it goes faster... <p> This module actually calculates the activations (so-called " +
...
// determine the proper format
    // NumElements = (Number of Neurons) * (Number of Examples)
    ...
    // and for the temporary thingamajig
    // NumElementsSmall = number of actual output neurons...
    long NumElementsSmall = (long)LayerTable.getValue(LayerTable.getDimensions()[0]-1,0) + 1;
    int FormatIndexSmall = -1; // initialize
    if (NumElementsSmall < NumberOfElementsThreshold) {
      // small means keep it in core; single dimensional in memory is best
...
//        System.out.println("SumTemp = " + SumTemp + "; StretchedSum = " + StretchedSum);
    ...
    for (long ExampleIndex = 0; ExampleIndex < nExamples; ExampleIndex++) {
      ...
      // Beware the MAGIC NUMBER!!! we are doing the input layer case, so the layer is, in fact, zero...
      ThisLayerFirst = (long)LayerStartFinishNeuronTable.getValue(0,0);
      ThisLayerLast = (long)LayerStartFinishNeuronTable.getValue(0,1);

//      System.out.println("ExampleIndex = " + ExampleIndex + "; ThisLayerFirst = " + ThisLayerFirst + "; ThisLayerLast = " + ThisLayerLast);

      for (long NeuronIndex = ThisLayerFirst; NeuronIndex < ThisLayerLast + 1; NeuronIndex++) {
//        System.out.println("ExampleIndex = " + ExampleIndex + "; NeuronIndex = " + NeuronIndex);
...
//        PreviousLayerLast = (long)LayerStartFinishNeuronTable.getValue(LayerIndex - 1, 1);
      ...
      MultiFormatMatrix OutputExpedSums = new MultiFormatMatrix(FormatIndexSmall,
