...
public class MultiEntropyCondensed extends ComputeModule {

  public String getModuleName() {
    return "MultiEntropyCondensed";
  }
  ...
  public String getModuleInfo() {
    return "This module calculates the cross-entropy error function " +
        ...
        "outcome, it will count as a correct prediction. <p> This uses the activations " +
        "from the Condensed versions of the NNActivations which do not list the input neurons.";
  }
...
// pull out some relevant constants
    ...
    long nNeuronsListed = NeuronActivations.getDimensions()[1];
    long nLayers = LayerTable.getDimensions()[0];
    long nOptions = (long) LayerTable.getValue(nLayers - 1, 0) + 1; // don't forget the extra one
    long FirstOutput = nNeuronsListed - nOptions;

// begin to do the deeds
    double ActivationRealized = -1.0;
    double EntropyTemp = -2.0;
    long nPredictedCorrectly = -3;
    long RealizedIndex = -4;
...
//      System.out.println("  ExampleIndex = " + ExampleIndex + "; nExamples = " + nExamples );
      RealizedIndex = FirstOutput + (long)Targets.getValue(ExampleIndex, 0);
      RealizedIsMaximum = true;
      ActivationRealized = NeuronActivations.getValue(ExampleIndex, RealizedIndex);
      for (long OptionIndex = 0; OptionIndex < nOptions; OptionIndex++) {
