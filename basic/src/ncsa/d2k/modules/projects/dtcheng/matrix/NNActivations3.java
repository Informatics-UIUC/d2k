...
public class NNActivations3
    extends ComputeModule {

  public String getModuleName() {
    return "NNActivations3";
  }

  public String getModuleInfo() {
    return "This is another attempt to speed things up. <p>This module actually calculates the activations (so-called " +
...
//    System.out.println("the very beginning of the neuron activations. nothing has been pulled in yet.");
...
// begin to do the deeds
    ...
    // Beware the MAGIC NUMBER!!! bounds are now hard coded instead of using sensible names...
    for (long NeuronIndex = 0; NeuronIndex < (long)LayerTable.getValue(0,0); NeuronIndex++) {
...
//        System.out.println("ExampleIndex = " + ExampleIndex + "; NeuronIndex = " + NeuronIndex + "; Value = " + NeuronActivations.getValue(ExampleIndex,NeuronIndex));
      }
    }

    // activate the subsequent hidden layers
    // nLayers - 1 because we'll do the final layer separately
    long ThisLayerFirst = -2;
//    long ThisLayerLast = -3;
...
//      System.out.println("LayerIndex = " + LayerIndex + "; ThisLayerFirst = " + ThisLayerFirst);
...
//          NeuronUnderConsideration = ThisLayerFirst + WithinIndex;
          SumTemp = Biases.getValue(ThisLayerFirst + WithinIndex, 0);
          WeightShifter = FirstWeightNumber + WithinIndex;
        //          for (long IncomingIndex = 0; IncomingIndex < nNeuronsInPreviousLayer; IncomingIndex++) {
        for (long IncomingIndex = 0; IncomingIndex < (long)LayerTable.getValue(LayerIndex - 1,0); IncomingIndex++) {
//          WeightUnderConsideration = WeightShifter + IncomingIndex*nNeuronsInThisLayer;

  //          PreviousActivationUnderConsideration =
  //              NeuronActivations.getValue(ExampleIndex, PreviousLayerFirst + IncomingIndex);
            SumTemp += NeuronActivations.getValue(ExampleIndex, PreviousLayerFirst + IncomingIndex) *
                SerializedWeights.getValue(WeightShifter + IncomingIndex*nNeuronsInThisLayer, 0);
          }
...
//          NeuronActivations.setValue(ExampleIndex, NeuronUnderConsideration, SquashedSum);
          NeuronActivations.setValue(ExampleIndex, ThisLayerFirst + WithinIndex, 1.0/(1.0 + java.lang.Math.exp(-SumTemp)));
//          System.out.println("ExampleIndex = " + ExampleIndex + "; NeuronIndex = " + (long)(ThisLayerFirst + WithinIndex) + "; Value = " + NeuronActivations.getValue(ExampleIndex,ThisLayerFirst + WithinIndex));
        }
      }
    }
...
//        System.out.println("SumTemp = " + SumTemp + "; Exped = " + java.lang.Math.exp(SumTemp) );
        OutputExpedSums.setValue(0, WithinIndex, StretchedSum); // record the numerator
...
//                                   OutputExpedSums.getValue(0,WithinIndex)/Denominator);
        NeuronActivations.setValue(ExampleIndex, ThisLayerFirst + WithinIndex,
                                   (OutputExpedSums.getValue(0,WithinIndex)/Denominator ) );
//        System.out.println("ExampleIndex = " + ExampleIndex + "; NeuronIndex = " + (long)(ThisLayerFirst + WithinIndex) + "; Value = " + NeuronActivations.getValue(ExampleIndex,ThisLayerFirst + WithinIndex));
      }
      // do the final "permanently-zeroed" neuron
      NeuronActivations.setValue(ExampleIndex, nNeurons, (1.0/Denominator) );
//      System.out.println("ExampleIndex = " + ExampleIndex + "; NeuronIndex = " + nNeurons + "; Value = " + NeuronActivations.getValue(ExampleIndex,nNeurons));
    }
