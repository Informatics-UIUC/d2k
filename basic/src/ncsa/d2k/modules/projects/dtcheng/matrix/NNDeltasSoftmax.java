...
// determine the proper format
    long NumElements = (NeuronActivations.getDimensions()[0] *
...
// pull out some relevant constants
    ...
    long nOutputs = (long) LayerTable.getValue(nLayers - 1, 0); // this is the number of relevant outputs; that is, excluding the standardized one
...
// begin to do the deeds
    ...
    MultiFormatMatrix NNDeltas = new MultiFormatMatrix(FormatIndex,
        new long[] {nExamples, nNeurons}); // ignoring the extra un-notated output neuron
    long MyLayer = -1;
    long MyLayerFirst = -2;
    ...
    for (long ExampleIndex = 0; ExampleIndex < nExamples; ExampleIndex++) {
      ...
      for (long StorageIndex = 0; StorageIndex < nOutputs; StorageIndex++) {
        if ((long) Targets.getValue(ExampleIndex, 0) == StorageIndex) {
...
//      System.out.println("OutputFirst = " + OutputFirst);
      for (long NeuronIndex = (OutputFirst - 1); NeuronIndex >= 0; NeuronIndex--) {
//        System.out.println("NeuronIndex = " + NeuronIndex);
        ...
        MyLayerFirst = (long)LayerStartFinishNeuronTable.getValue(MyLayer, 0);
        NextLayerFirst = (long)LayerStartFinishNeuronTable.getValue(MyLayer + 1, 0);
        ...
        FirstWeightNumber = 0;
        for (long LayerTempIndex = 0; LayerTempIndex < MyLayer; LayerTempIndex++) {
          FirstWeightNumber += ((long) LayerTable.getValue(LayerTempIndex, 0) *
                                (long) LayerTable.getValue(LayerTempIndex + 1, 0));
        }
        FirstWeightNumber += ((NeuronIndex - MyLayerFirst) *
                              (long) LayerTable.getValue(MyLayer + 1, 0));
        PosteriorSum = 0;
