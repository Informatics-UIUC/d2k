...
// Neuron to Layer table
    ...
    for (long LayerIndex = 0; LayerIndex < nLayers; LayerIndex++) {
      NewStart = PreviousEnd + 1;
      NewEnd = NewStart + (long)LayerTable.getValue(LayerIndex,0);
...
// Layer Starting/Finishing Neuron Table
    ...
    LayerStartFinishNeuronTable.setValue(0, 1, (long)LayerTable.getValue(0,0) - 1);
    for (long LayerIndex = 1; LayerIndex < nLayers; LayerIndex++) {
      for (long RowIndex = 0; RowIndex < LayerIndex; RowIndex++) {
        FirstSum += LayerTable.getValue(RowIndex,0);
      }
      for (long RowIndex = 0; RowIndex < LayerIndex + 1; RowIndex++) {
