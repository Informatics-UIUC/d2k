...
public class ReadLineByteStreamFromInput extends InputModule {
	...
	public String getModuleName() {
		return "ReadLineByteStreamFromInput";
	}
	...
	public void doit() throws Exception {
		...
		if (InitialExecution) {
			...
			Input = new FlatFile(file, "r", ReadBufferSize,
					true);
		...
		if (((StartLineIndex == -1) || ((LineIndex >= StartLineIndex)))
			...
			if (Trace && (LineIndex % ReportInterval == 0))
				System.out.println("line "
						+ LineIndex
						+ " = "
						+ new String(Input.Buffer, Input.LineStart,
								(Input.LineEnd - Input.LineStart - 1)));
