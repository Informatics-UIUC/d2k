...
public class InstanceBasedInducerOpt extends FunctionInducerOpt {
  ...
  private double  DistanceWeightingPower;
  private double  ZeroDistanceWeight;
  private boolean UseStepwiseRegression;
  private int     Direction;
  private int     NumRounds;
  private boolean ExpandNeighborhoodForTies;
  private double  MaxExpansionFactor;
  ...
  public void setControlParameters(ParameterPoint parameterPoint) {
    ...
    NeighborhoodSize       = (int) parameterPoint.getValue(i++);
    DistanceWeightingPower =       parameterPoint.getValue(i++);
    ZeroDistanceWeight     =       parameterPoint.getValue(i++);

    if (parameterPoint.getValue(i++) < 0.5) {
      UseStepwiseRegression = false;
    } else {
      UseStepwiseRegression = true;
    }
    ...
    if (parameterPoint.getValue(i++) < 0.5) {
      ExpandNeighborhoodForTies = false;
    } else {
      ExpandNeighborhoodForTies = true;
    }
    
    
    MaxExpansionFactor = parameterPoint.getValue(i++);
