...
import ncsa.d2k.modules.weka.filters.Option;
import ncsa.d2k.modules.weka.filters.OptionHandler;
import ncsa.d2k.modules.weka.filters.Range;
import ncsa.d2k.modules.weka.filters.Utils;
import ncsa.d2k.modules.weka.filters.search.ASSearch;
import ncsa.d2k.modules.weka.filters.search.RankedOutputSearch;
...
//AS-temp  private Remove m_attributeFilter = null;
   ...
   */
   /*AS-temp
  public ExampleTable reduceDimensionality(ExampleTable in) throws Exception {
    if (m_attributeFilter == null) {
      throw new Exception("No feature selection has been performed yet!");
    }

    if (m_transformer != null) {
      ExampleTable transformed = new ExampleTable(m_transformer.transformedHeader(),
					    in.getNumRows());
...
*/
   ...
   */
  /*AS-temp
  public double [] reduceDimensionality(double [] in) throws Exception {
    if (m_attributeFilter == null) {
      throw new Exception("No feature selection has been performed yet!");
    }
    if (m_transformer != null) {
    ...
    double [] result = m_attributeFilter.output();
    return result;
  }
*/
  ...
  public AttributeSelection () {
    ...
    //AS-temp setEvaluator(new CfsSubsetEval());
    //AS-temp setSearch(new ForwardSelection());
  ...
  public String CVResultsString () throws Exception {
    ...
    int fieldWidth = (int)(Math.log(m_trainTable.getNumColumns()) +1.0);
	...
	(m_trainTable.isColumnNominal(m_trainTable.getOutputFeatures()[0]))) {
	  ...
	  CvString.append(Utils.doubleToString(Math.
			...
			+"  "
			  + Utils.doubleToString(((double)(s[i] + 1)),
						 fieldWidth, 0)
			  + " "
			  + m_trainTable.getColumnLabel(s[i])
			  + "\n");
	}
      }
    }
	...
	if ((m_ASEvaluator instanceof UnsupervisedSubsetEvaluator) ||
	    (i != m_trainTable.getOutputFeatures()[0])) {
   ...
   */
  public void selectAttributesCVSplit(ExampleTable split) throws Exception {
    ...
    if (m_rankResults == null && m_subsetResults == null) {
      m_subsetResults = new double[split.getNumColumns()];
      m_rankResults = new double[4][split.getNumColumns()];
    }
  ...
  public String CrossValidateAttributes () throws Exception {
    ExampleTable cvData = new ExampleTable(m_trainTable);
    ExampleTable train;
    double[][] rankResults;
...
*/
   ...
   */
  public void SelectAttributes (ExampleTable data) throws Exception {
    int [] attributeSet;
...
*/
	...
	m_ASEvaluator instanceof UnsupervisedSubsetEvaluator) {
      ...
      if (m_trainTable.getOutputFeatures() == null) {
      	int  defaultClassIndex =  m_trainTable.getNumColumns()-1;
      	int [] outputs = { defaultClassIndex };
	m_trainTable.setOutputFeatures(outputs);
      }
    }


    // Initialize the attribute evaluator
    m_ASEvaluator.buildEvaluator(m_trainTable);
    /*AS-temp
...
*/
	...
	m_selectionResults.
	  append(Utils.doubleToString(m_attributeRanking[i][1],
		 ...
		 + Utils.doubleToString((m_attributeRanking[i][0] + 1),
					fieldWidth+1,0)
		 + " "
		 + m_trainTable.getColumnLabel((int)m_attributeRanking[i][0])
		 + "\n");
      }
	  ...
	  && !(m_ASEvaluator instanceof UnsupervisedAttributeEvaluator))
	{
	  ...
	  m_selectedAttributeSet[m_numToSelect] =
	    m_trainTable.getOutputFeatures()[0];
	}
      else {
	...
	if (i == (attributeSet.length - 1)) {
      ...
      for (int i=0;i<attributeSet.length;i++) {
	m_selectionResults.append("                     "
				  +m_trainTable.getColumnLabel(attributeSet[i])
				  +"\n");
      }
    }
...
/*AS-temp
    ...
    if (m_selectedAttributeSet != null && !m_doXval) {
      ...
      m_attributeFilter.setInputFormat(m_trainTable);
    }

    // Save space
    m_trainTable = new ExampleTable(m_trainTable, 0);
*/
  }
    ...
    throws Exception
  {
	...
	if (classString.equals("first")) {
	  ...
	  classIndex = train.getNumColumns();
	} else {
      ...
      if ((classIndex != -1) &&
	  ((classIndex == 0) || (classIndex > train.getNumColumns()))) {
	throw  new Exception("Class index out of range.");
      }

      if (classIndex != -1) {
      	int defaultClassIndex = classIndex -1;
      	int [] outputs = {defaultClassIndex };
	train.setOutputFeatures(outputs);
      }
      else {
	//	classIndex = train.getNumColumns();
	...
	if (breakLoc != -1) {
	  ...
	  //TODO searchOptions = Utils.splitOptions(searchOptionsString);
	}
      }
      else {
	try {
    ...
    System.out.println("going to select attributes");
  ...
  private String printSelectionResults () {
	...
	&& !(m_ASEvaluator instanceof UnsupervisedAttributeEvaluator)) {
      ...
      if (m_trainTable.isColumnNumeric(m_trainTable.getOutputFeatures()[0])) {
  	text.append("numeric): ");
      }
      else {
	text.append("nominal): ");
      ...
      text.append((m_trainTable.getOutputFeatures()[0] + 1)
		  + " "
		  + m_trainTable.getColumnLabel(m_trainTable.getOutputFeatures()[0])
		  + "):\n");
    }
    else {
