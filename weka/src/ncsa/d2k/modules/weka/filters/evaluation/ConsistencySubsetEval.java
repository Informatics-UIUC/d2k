...
public class ConsistencySubsetEval extends SubsetEvaluator {
  ...
  public class hashKey {
     ...
     */
    public hashKey(int t, int numAtts) throws Exception {
	...
	if (i == cindex) {
	  ...
	  if ((missing[i] = m_trainTable.isValueMissing(t,i)) == false) {
	    attributes[i] = m_trainTable.getDouble(t,i);
	  }
	}
      }
    }
     ...
     */
   /* public String toString(Table t, int maxColWidth) {

      int i;
      int cindex = t.getOutputFeatures()[0];
...
//TODO maybe reintroduce this transformation
    ...
    String[] attributeCounts = TableUtilities.uniqueValues(m_trainTable,m_classIndex);
    m_numClasses = attributeCounts.length;
  ...
  public double evaluateSubset (BitSet subset) throws Exception {
	...
	if (m_trainTable.isValueMissing(i,fs[j])) {
	  instArray[j] = Double.MAX_VALUE;
	} else {
	  instArray[j] = m_trainTable.getDouble(i,fs[j]);
	}
      }
      insertIntoTable(i, instArray);
    }
   ...
   */
  private void insertIntoTable(int i, double [] instA)
       throws Exception {
    ...
    if (tempClassDist2 == null) {
      newDist = new double [m_numClasses];
      newDist[m_trainTable.getInt(i,m_classIndex)] = 1;//inst.weight();

      // add to the table
      m_table.put(thekey, newDist);
    } else {
      // update the distribution for this instance
      tempClassDist2[m_trainTable.getInt(i,m_classIndex)] += 1;//inst.weight();
