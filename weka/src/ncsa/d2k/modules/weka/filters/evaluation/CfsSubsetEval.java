...
import ncsa.d2k.modules.weka.filters.ContingencyTables;
import ncsa.d2k.modules.weka.filters.EntropyBinning;
  ...
  implements OptionHandler
{
  ...
  private EntropyBinning m_disTransform;
  /** The class index */
...
//TODO enforce missing values to be removed before this is called
...
// do denominator
  ...
  private float correlate (int att1, int att2) {
    ...
    boolean att1_is_num = (m_trainTable.isColumnScalar(att1));
    boolean att2_is_num = (m_trainTable.isColumnScalar(att2));
  ...
  private double symmUncertCorr (int att1, int att2) {
   ...
   //TODO better counts....
   // ni = m_trainTable.attribute(att1).numValues() + 1;
   // nj = m_trainTable.attribute(att2).numValues() + 1;
    String[] attributeCounts = TableUtilities.uniqueValues(m_trainTable,att1);
    int numAttributes = attributeCounts.length;
    ni = numAttributes + 1;
    attributeCounts = TableUtilities.uniqueValues(m_trainTable,att2);
    numAttributes = attributeCounts.length;
    nj =  numAttributes + 1;
    ...
    for (i = 0; i < m_numInstances; i++) {
      //inst = m_trainTable.instance(i);

      if (m_trainTable.isValueMissing(i,att1)) {
	ii = ni - 1;
      }
      else {
	ii = m_trainTable.getInt(i,att1);
      }

      if (m_trainTable.isValueMissing(i,att2)) {
	jj = nj - 1;
      }
      else {
	jj = m_trainTable.getInt(i,att2);
      }
  ...
  private double meanOrMode(Table table,int att) {
   if (table.isColumnNominal(att)) {
      String val = TableUtilities.mode(table,att);
      return (double)Integer.parseInt(val);
   } else
      return TableUtilities.mean(table,att);
  }
  ...
  private double num_num (int att1, int att2) {
    ...
    double mx = meanOrMode(m_trainTable,att1);
    double my = meanOrMode(m_trainTable,att2);

    for (i = 0; i < m_numInstances; i++) {
      diff1 = (m_trainTable.isValueMissing(i,att1)) ? 0.0 : (m_trainTable.getDouble(i,att1) - mx);
      diff2 = (m_trainTable.isValueMissing(i,att2)) ? 0.0 : (m_trainTable.getDouble(i,att2) - my);
      num += (diff1*diff2);
      sx += (diff1*diff1);
      sy += (diff2*diff2);
    }
  ...
  private double num_nom2 (int att1, int att2) {
    int i, ii, k;
    double temp;
    int mx = (int)meanOrMode(m_trainTable,att1);
    double my = meanOrMode(m_trainTable,att2);
    double stdv_num = 0.0;
    double diff1, diff2;
    double r = 0.0, rr, max_corr = 0.0;
    int numOfValues = TableUtilities.getNumOfValues(m_trainTable,att1);
    int nx = (!m_missingSeperate) ? numOfValues : numOfValues + 1;
    ...
    for (k = 0; k < m_numInstances; k++) {
      //inst = m_trainTable.instance(k);
      // std dev of numeric attribute
      diff2 = (m_trainTable.isValueMissing(k,att2)) ? 0.0 : ( m_trainTable.getDouble(k,att2) - my);
      stdv_num += (diff2*diff2);

      //
      for (i = 0; i < nx; i++) {
	if (m_trainTable.isValueMissing(k,att1)) {
	  if (!m_missingSeperate) {
	    temp = (i == mx)? 1.0 : 0.0;
	  }
	  else {
	  ...
	  temp = (i ==   m_trainTable.getDouble(k,att1))? 1.0 : 0.0;
	}
  ...
  private double nom_nom (int att1, int att2) {
    ...
    double temp1, temp2;
    int mx = (int)meanOrMode(m_trainTable,att1);
    int my = (int)meanOrMode(m_trainTable,att2);
    double diff1, diff2;
    double r = 0.0, rr, max_corr = 0.0;
    int numOfValues = TableUtilities.getNumOfValues(m_trainTable,att1);
    int nx = (!m_missingSeperate) ? numOfValues : numOfValues + 1;

    numOfValues = TableUtilities.getNumOfValues(m_trainTable,att2);
    int ny = (!m_missingSeperate) ? numOfValues : numOfValues + 1;
    ...
    for (z = 0; z < m_numInstances; z++) {
      //inst = m_trainTable.instance(z);

      for (j = 0; j < ny; j++) {
	if (m_trainTable.isValueMissing(z,att2)) {
	  if (!m_missingSeperate) {
	    temp2 = (j == my)? 1.0 : 0.0;
	  }
	  else {
	  ...
	  temp2 = (j == m_trainTable.getDouble(z,att2))? 1.0 : 0.0;
	}
	...
	stdvsy[j] += (diff2*diff2);
      ...
      //
      for (i = 0; i < nx; i++) {
	if (m_trainTable.isValueMissing(z,att1)) {
	  if (!m_missingSeperate) {
	    temp1 = (i == mx)? 1.0 : 0.0;
	  }
	  else {
	  ...
	  temp1 = (i == m_trainTable.getDouble(z,att1))? 1.0 : 0.0;
	}
