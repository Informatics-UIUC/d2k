...
import ncsa.d2k.modules.weka.filters.FastVector;
...
import ncsa.d2k.modules.weka.filters.StartSetHandler;
import ncsa.d2k.modules.weka.filters.Utils;
import ncsa.d2k.modules.weka.filters.evaluation.ASEvaluation;
import ncsa.d2k.modules.weka.filters.evaluation.SubsetEvaluator;
import ncsa.d2k.modules.weka.filters.evaluation.UnsupervisedSubsetEvaluator;
...
*/
    ...
    throws Exception
  {
    ...
    if (optionString.length() != 0) {
      ...
      setDirection(SELECTION_FORWARD);
    }
    ...
    throws Exception
  {
    ...
    if (ASEval instanceof UnsupervisedSubsetEvaluator) {
      ...
      m_hasClass = true;
      m_classIndex = ((ExampleTable) data).getOutputFeatures()[0];
    }

    SubsetEvaluator ASEvaluator = (SubsetEvaluator)ASEval;

    m_numAttribs = data.getNumColumns();
    ...
    m_startRange.setUpper(m_numAttribs); // m_numAttribs -1
    if (!(getStartSet().equals(""))) {
    ...
    if (m_starting != null) {
				...
				if (m_searchDirection == SELECTION_BACKWARD) {
						...
						for (int i = 0, j = 0; i < inputFeatures.length; i++) {
								if (i != m_classIndex) {
										best_group.set(inputFeatures[i]);
										m_starting[j++] = inputFeatures[i];
								}
						}
