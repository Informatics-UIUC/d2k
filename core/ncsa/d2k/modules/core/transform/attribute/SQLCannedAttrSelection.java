...
public class SQLCannedAttrSelection extends DataPrepModule {
	...
	*/
	public String getOutputInfo(int i) {
		switch (i) {
			case 0 :
				return "Contains binning info";
			case 1 :
				return "table containing metadata";
			case 2 :
				return "name of the table containing the actual data";
			default :
	...
	public void doit() {
		...
		for (int id = 0; id < sz; id++) {
			...
			try {
				...
				ResultSet maxVal =
					stmt.executeQuery(
						"SELECT MAX(" + attName + ") FROM " + tableName);
				...
				ResultSet minVal =
					stmt.executeQuery(
						"SELECT MIN(" + attName + ") FROM " + tableName);
			...
			for (int q = 0; q < numBins; q++) {
				...
				try {
					if (q == 0)
						binTree.addNumericBin(
							attName,
							nameBuffer.toString(),
							BinTree.LESS_THAN_EQUAL_TO,
							upper);
					else if (q == (numBins - 1))
						binTree.addNumericBin(
							attName,
							nameBuffer.toString(),
							BinTree.GREATER_THAN,
							lower);
					else {
					...
					BinEntry be =
						new BinEntry(attName, nameBuffer.toString(), NUMERIC);
