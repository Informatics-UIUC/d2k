...
public class ADTDefineBins extends DefineBins {
	...
	*/
	public String getInputInfo(int i) {
		switch (i) {
			case 0 :
				return "ADTree containing counts";
			case 1 :
				return "Table with metadata in it .";
			default :
	...
	*/
	public String getOutputInfo(int i) {
		switch (i) {
			case 0 :
				return "Bin tree with binning info";
			case 1 :
				return " metadata table, unchanged";
			default :
	...
	*/
	class SQLBinView extends BinView {
		...
		void addNumericUniformRange() {
			System.err.println(
				"addNumericUniformRange not implemented for an ADTree  ");
		}
		/*
		  void addNumericUniformRange() {
		...
		*/
		void autoStringBin() {
			...
			for (int z = 0; z < selected.length; z++) {
				...
				int attidx =
					((Integer) indexLookup.get(attName.trim())).intValue();
				...
				while (i.hasNext()) {
					...
					try {
						...
						JOptionPane.showMessageDialog(
							this,
							"Unable to proceed.  Please add bins manually.",
							"Error",
							JOptionPane.ERROR_MESSAGE);
						return;
					}
