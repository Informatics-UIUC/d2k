...
public class TreeScrollPane extends JScrollPane {
	...
	public TreeScrollPane(DecisionTreeModel mdl, DecisionTreeNode tree, BrushPanel brushpanel) {
		treepanel = new TreePanel(mdl, tree, brushpanel);
	...
	*/
	public class TreePanel extends JPanel implements MouseListener, MouseMotionListener {
		...
		public TreePanel(DecisionTreeModel mdl, DecisionTreeNode tree, BrushPanel brushpanel) {
			this.model = mdl;
			this.tree = tree;
		...
		protected void getMaxDepth(DecisionTreeNode node) {
			/*if(parent != null)
			...
			if(widthMap.get(new Integer(dep)) == null) {
				widthMap.put(new Integer(dep), new Integer(1));
			}
			else {
				Integer tal = (Integer)widthMap.get(new Integer(dep));
				widthMap.put(new Integer(dep), new Integer(tal.intValue() + 1));
			}

			if(dep > maxDepth)
				maxDepth = dep;
			for(int i = 0; i < node.getNumChildren(); i++)
				getMaxDepth(node.getChild(i));
		}
		...
		*/
		public void buildViewTree() {
			/*treeheight = tree.getHeight()*/getMaxDepth(tree);
			treeheight = maxDepth;
			//leaves = (int) Math.pow(2.0, (double) treeheight);
			int leaves = 0;
			Iterator iter = widthMap.keySet().iterator();
			while(iter.hasNext()) {
				Integer tal = (Integer)iter.next();
				if(tal.intValue() > leaves)
					leaves = tal.intValue();
			}
			leaves = (int)Math.pow(2.0, leaves);
		...
		public void buildRecursive(DecisionTreeNode modelnode, ViewNode viewnode) {
			...
			int level = /*modelnode.getLevel()*/maxDepth-modelnode.getDepth();
			double div = Math.pow(2.0, (double) (level+2));
			double increment = width/div;

			if (/*modelnode.getLeft()*/modelnode.getChild(0) != null) {
				ViewNode leftview = new ViewNode(model, /*modelnode.getLeft()*/
				...
				level = /*modelnode.getLeft().getLevel();*/
					maxDepth-modelnode.getChild(0).getDepth();
				LinkedList levellist = offsets[level];
				levellist.add(leftview);
			}
			if (/*modelnode.getRight()*/modelnode.getChild(0) != null) {
				ViewNode rightview = new ViewNode(model,/*modelnode.getRight()*/
				...
				level = /*modelnode.getRight().getLevel()*/
					maxDepth-modelnode.getChild(1).getDepth();
				LinkedList levellist = offsets[level];
		...
		public void drawRecursive(Graphics2D g2, DecisionTreeNode modelnode, ViewNode viewnode) {

			if(!modelnode.isLeaf()) {
			if (modelnode.getChild(0) != null) {
				if (!viewnode.collapsed) {
					ViewNode leftview = viewnode.left;
					drawLine(g2, modelnode, /*modelnode.getLeftRule()*/
			...
			if (modelnode.getChild(1) != null) {
				if (!viewnode.collapsed) {
					...
					drawLine(g2, modelnode, /*modelnode.getRightRule()*/
		...
		public void drawLine(Graphics2D g2, DecisionTreeNode node, String rule, double x1, double y1, double x2, double y2) {
			int linestroke = 1;
