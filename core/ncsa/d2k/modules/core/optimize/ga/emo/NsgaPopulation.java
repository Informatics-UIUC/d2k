	...
	*/
	public NsgaPopulation (Range [] ranges, ObjectiveConstraints [] objConstraints,
		...
		if (ranges [0] instanceof BinaryRange) {
			...
			for (int i = 0 ; i < numMembers ; i++) {
				members[i] = new MONumericIndividual ((DoubleRange []) ranges, objConstraints);
				nextMembers[i] = new MONumericIndividual ((DoubleRange []) ranges, objConstraints);
			}
		...
		if (debug) {
			for (int i = 0; i < popSize; i++) {
				...
				for (int j = 0 ; j < numIndDom [i] ; j++)
					System.out.println("   "+indDomByMe[i][j]+"-"+theGuys[indDomByMe[i][j]]);
			}
			System.out.println();
		}
