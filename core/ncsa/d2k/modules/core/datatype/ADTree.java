...
public class ADTree extends Node {
    ...
    */
    public void setLabel(int index, String value) {
			if(debug1) System.out.println("set labels " + index + " " + value );
			labels[index] = value;
    ...
    */
    public void initializeFromVT(Table vt) {
	  ...
	  for(int i = 0 ; i < vt.getNumRows(); i ++ ) {
	    node = this;
	    node.count++;
	    if (debug1) System.out.println(" MAIN COUNT : " + node.count + " " + this);
		  ...
		  if(varNode.containsKey(vt.getString(i,j-1)))
		      ...
		      if(debug2)
					 System.out.println("hm does not contain key "
		      		       + vt.getString(i,j-1)+" inserted it j =" + j);
		  }
		  node.count++;
	    }
		}
    ...
    */
    public void countLine(Node nd, String[] values) {
	  ...
	  node.count++;
	   if(debug1) System.out.println(" MAIN COUNT : " + node.count + " " + node);
	  ...
	  for ( int j = 1; j <= values.length; j ++) {
	    ...
	    if(varNode.containsKey(values[j-1]))
			  ...
			  if (debug2) System.out.println("hm does not contain key "
				       + values[j-1]+" inserted it j =" + j);
	    }
    ...
    */
	...
	for ( int j = 1; j <= len; j ++) {
		...
		node.putVarNode(j,hm);
		if(debug2) System.out.println("created and inserted hashmap for  "
				       + value + " with j=" + j);
	    }
	    varNode = node.getVarNode(j);
    ...
    */
	...
	if(debug2) System.out.println("EXPANDNODE " + index + " " + node);
	HashMap previousVarNode = node.getVarNode(index-1);

	if(debug2) System.out.println("previousVarNode is " + previousVarNode);
	...
	while(values.hasNext()) {
	    ...
	    if(debug2)  System.out.println("hmc is " + hmc);
	    hm = node.addHashMaps(hm, hmc);
	}
	node.putVarNode(index,hm);
	if(debug1)	System.out.println("EXPANDED NODE IS "  + node);
    ...
    */
	...
	while(it.hasNext()) {
	    ...
	    if (varNode == null) {
		System.err.println("ADTree is not expanded: Trying to expand");
		if(debug1) System.out.println("node to expand " + node);
		expand(node);
    ...
    /*
      ...
      @param pos -position where the serach for the node containg
    ...
    */
	...
	while(it.hasNext()) {
	    ...
	    Node node = findNode(position+1,index,(Node)varNode.get(key),value);
	    if(node != null) {
