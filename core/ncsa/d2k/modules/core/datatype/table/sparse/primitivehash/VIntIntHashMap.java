...
//import ncsa.d2k.modules.projects.vered.sparse.column.SparseIntColumn;
...
public class VIntIntHashMap extends TIntIntHashMap implements VHashMap{


    public static final int NOT_EXIST = SparseIntColumn.NOT_EXIST;
/**
 ...
 /**
   ...
   */
   private VIntIntHashMap getSortedOrder(int[] validKeys, int[] values){
     ...
     */
   private int getNewKey(int currVal, int[] values, int key, boolean[] ocuupiedIndices){
    int retVal = -1;
    ...
    public int remove(int key) {
        int prev = SparseIntColumn.NOT_EXIST;
        int index = index(key);
        if (index >= 0) {
            prev = _values[index];
