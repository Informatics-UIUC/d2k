...
//import ncsa.d2k.modules.projects.vered.sparse.column.SparseShortColumn;
...
public class VIntShortHashMap extends TIntHash implements VHashMap{
  /**
   ...
   */
    ...
    public static final short NOT_EXIST = SparseShortColumn.NOT_EXIST;
    ...
    public short remove(int key) {
        short prev = SparseShortColumn.NOT_EXIST;
        int index = index(key);
        if (index >= 0) {
            prev = _values[index];
    ...
    private static final class EqProcedure implements VIntShortProcedure {
 ...
 /**
   ...
   */
   private VIntIntHashMap getSortedOrder(int[] validKeys, short[] values){
     ...
     */
   private int getNewKey(short currVal, short[] values, int key, boolean[] ocuupiedIndices){
    int retVal = -1;
