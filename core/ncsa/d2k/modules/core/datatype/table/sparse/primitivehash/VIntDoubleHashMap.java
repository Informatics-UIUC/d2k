...
//import ncsa.d2k.modules.projects.vered.sparse.column.SparseDoubleColumn;
...
public class VIntDoubleHashMap extends TIntDoubleHashMap implements VHashMap{
    ...
    public static final double NOT_EXIST = SparseDoubleColumn.NOT_EXIST;
 ...
 /**
   ...
   */
   private VIntIntHashMap getSortedOrder(int[] validKeys, double[] values){
     ...
     */
   private int getNewKey(double currVal, double[] values, int key, boolean[] ocuupiedIndices){
    int retVal = -1;
    ...
    public double remove(int key) {
        double prev = SparseDoubleColumn.NOT_EXIST;
        int index = index(key);
        if (index >= 0) {
            prev = _values[index];
