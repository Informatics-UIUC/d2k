...
//import ncsa.d2k.modules.projects.vered.sparse.column.SparseLongColumn;
...
public class VIntLongHashMap extends TIntLongHashMap implements VHashMap{

    public static final long NOT_EXIST = SparseLongColumn.NOT_EXIST;
 ...
 /**
   ...
   */
   private VIntIntHashMap getSortedOrder(int[] validKeys, long[] values){
     ...
     */
   private int getNewKey(long currVal, long[] values, int key, boolean[] ocuupiedIndices){
    int retVal = -1;
    ...
    public long remove(int key) {
        long prev = SparseLongColumn.NOT_EXIST;
        int index = index(key);
        if (index >= 0) {
            prev = _values[index];
