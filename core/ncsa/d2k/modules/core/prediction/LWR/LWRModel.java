...
/**
  ...
  @author talumbau
 */
public class LWRModel
    extends PredictionModelModule
    implements Serializable {

  //properties
  private int kernelSelector;

  private int distanceSelector;
  private int nearestNeighbors;
  private boolean useNearestNeighbors;
  private boolean NotEnoughPredictions; //special case where # of training predictions < rows of test data

  private int count; //counter for the rows of the Testtable
  private int num_pts; //number of points in the regression plots
  private int countPredCol;
  private int TotalNumPredictions; //number of predictions contained in training data
  private int N;
  private double minDist = -1;
  private int minDistIndex = 0;
  private double[] weights;
  private double[] kernels;
  private double[] distances;
  private double[] criterion;
  private ExampleTable Traintable;
  private MutableTable TraintableSubset;
//  private PredictionTable finalTable;
//  private Matrix X;
//  private Matrix Z;
//	NumericColumn y;  //actual query values for given data
  ...
  public String[] getOutputTypes() {
    String[] out = {
        "ncsa.d2k.modules.core.datatype.PredictionTable",
   ...
   */
  LWRModel(ExampleTable table1, int ker, int distance, int nearest,
           boolean useNearest, int numpts) {
    super(table1);
    Traintable = table1;
  ...
  public PredictionTable predict(Table table) {
    ExampleTable exTable;

    if(!(table instanceof ExampleTable))
      exTable = table.toExampleTable();
    else
      exTable = (ExampleTable)table;

    //TableImpl xTable = (TableImpl)generatePlotValues();
    Table xTable = generatePlotValues();
    ...
    ExampleTable tempTable = (ExampleTable) xTable.toExampleTable().copy();
    //tempTable.removeColumn(tempTable.getNumColumns() - 1);
    //finalTable = new PredictionTableImpl(tempTable);
    PredictionTable finalTable = tempTable.toPredictionTable();

    // Go through *once* for each input Column
    for (int i = 0; i < (N - 1); i++) {

      MutableTable TestTable = testTableLWRGen(xTable, i);
      ...
      Matrix X = new Matrix(constructX());
      ...
      for (int c = 0; c < numRows; c++) {
        ...
        Matrix Z = new Matrix(constructZ(X));
        ...
        try {
          ...
          for (int b = 0; b < TraintableSubset.getNumRows(); b++) {
            ...
            double xB = (arrayMult(p, BetaAr)) - y[b];
          ...
          System.out.println(
              "problem in solving Z...defaulting to nearest neighbor selection.");
  ...
  private double[][] constructX() {
    ...
    for (int i = 0; i < numC; i++) {
      ...
      for (int j = 0; j < numR; j++) {
        ...
        double d = TraintableSubset.getDouble(j, i);
        A[j][i] = d;
      }
    }
    return A;
   ...
   */
  private double[][] constructZ(Matrix B) {
  ...
  private Matrix constructV(double[] col) {
    double[][] matrix = new double[col.length][1];
    for (int i = 0; i < col.length; i++) {
      //matrix[i][0] = weights[i]*(((Number) col.getRow(i)).doubleValue());
      matrix[i][0] = weights[i] * col[i];
    }
   ...
   */
  private void constructWeights(Matrix D, double[] q) {
   ...
   */
  private void determineSubset(Table t) {
    //TraintableSubset = (MutableTable) DefaultTableFactory.getInstance().
    //    createTable();
    TraintableSubset = (MutableTable) t.copy();
    TraintableSubset.setNumColumns(0);
    ...
    for (int j = 0; j < t.getNumColumns(); j++) {
      ...
      TraintableSubset.setColumnLabel(Traintable.getColumnLabel(index),
                                      TraintableSubset.getNumColumns() - 1);
    }
  }
  ...
  /**
   ...
   @param String label
   */
  private int findIndexInTraintable(String label) {
    int num = -1;
   ...
   */
  private double getWeight(double[] x, double[] q) {
  ...
  /**
   ...
   @return double ker
   */
  private double kernelOf(double c) {
    double ker;
  ...
  private double[] removeYs(MutableTable table) {
    ...
    table.getColumn(retVal, numCol - 1);
   ...
   */
  private double[] getQuery(int c, Table Testtable) {
    ...
    for (int h = 0; h < numCol; h++) {
      ...
      query[h] = Testtable.getDouble(c, h);
    }
    return query;
  }

  /*  private double[][] subtractQuery(double[] q) {
  ...
  /**
   ...
   @param double x
   */
  private double kInverse(double x) {
    double y = 1 / x;
    if (y == Double.NaN) {
  ...
  /**
   ...
   @param double x
   */
  private double kInversePlusOne(double x) {
    double y = 1 / (x + 1);
    return y;
  }
  ...
  /**
   ...
   @param double x
   */
  private double kInverseSquare(double x) {
    double y = 1 / (x * x);
    return y;
  }
  ...
  /**
   ...
   @param double x
   */
  private double keSquare(double x) {
    double val = - (x * x);
  ...
  /**
   ...
   @param double x
   */
  private double kLinear(double x) {
    double y = 1 - x;
    return y;
  }
   ...
   */
  private double distUnweightedEuclidean(double[] x, double[] q) {
    int num1 = x.length;
    int num2 = q.length;
   ...
   */
  private void addConstant1s(MutableTable table) {
   ...
   */
  private MutableTable testTableLWRGen(Table xTble, int i) {
    ...
    //MutableTable t = (MutableTable) DefaultTableFactory.getInstance().
    //    createTable();
    MutableTable t = (MutableTable) xTble.copy();
    t.setNumColumns(0);
    t.addColumn(anArray);
    t.addColumn(predArray);
    t.setColumnLabel(xTble.getColumnLabel(i), 0);
    t.setColumnLabel(xTble.getColumnLabel(N - 1), 1);
  ...
  /**
   generatePlotValues()
   Uses the Traintable ExampleTable to generate a TableImpl
   ...
   @return double ans - the distance
   */
  private Table generatePlotValues() {
    ...
    /*    MutableTable returnTable = (MutableTable) DefaultTableFactory.getInstance().createTable();
        ...
        for (int i = 0; i < inputCols.length; i++) {
          ...
          double[] doubCol = fillXs(min, max, num_pts);
          //doubCol.setLabel(Traintable.getColumnLabel(inputCols[i]));
          returnTable.addColumn(doubCol);
          returnTable.setColumnLabel(Traintable.getColumnLabel(inputCols[i]),
                                     returnTable.getNumColumns() - 1);
        }
        double[] doubAr2 = new double[numRow];
        Traintable.getColumn(doubAr2, outputCols[0]);
        returnTable.addColumn(doubAr2);
        returnTable.setColumnLabel(Traintable.getColumnLabel(outputCols[0]),
                                   returnTable.getNumColumns() - 1);
//    ((TableImpl)returnTable).print();
        return returnTable;
     */

    ExampleTable et = (ExampleTable) Traintable.copy();

    int inputCols[] = et.getInputFeatures();
    int outputCols[] = et.getOutputFeatures();

    boolean[] toRemove = new boolean[et.getNumColumns()];
    for (int i = 0; i < toRemove.length; i++) {
      toRemove[i] = true;

    }
    for (int i = 0; i < inputCols.length; i++) {
      toRemove[inputCols[i]] = false;

    }
    toRemove[outputCols[0]] = false;

    et.removeColumnsByFlag(toRemove);

    for (int i = 0; i < inputCols.length; i++) {
      double min = getMin(et, numRow, inputCols[i]);
      double max = getMax(et, numRow, inputCols[i]);
      double[] doubCol = fillXs(min, max, num_pts);
      String lbl = et.getColumnLabel(inputCols[i]);
      et.setColumn(doubCol, inputCols[i]);
      et.setColumnLabel(lbl, inputCols[i]);
    }
  ...
  /**
   ...
   get the minimum value from a double[]
   */
  private double getMin(Table t, int numRow, int col) {
    double min = t.getDouble(0, col);
    for (int k = 1; k < numRow; k++) {
      double value = t.getDouble(k, col);
      if (value < min) {
        min = value;
      }
    }
  ...
  /**
   ...
   get the maximum value from a double[]
   */
  private double getMax(Table t, int numRow, int col) {
    double max = t.getDouble(0, col);
    for (int k = 1; k < numRow; k++) {
      double value = t.getDouble(k, col);
      if (value > max) {
        max = value;
  ...
  /**
   ...
   @param int numval
   */
  private double[] fillXs(double min, double max, int numval) {
  ...
  private double[] fillZeros(double[] ar, int newLength) {
