...
import ncsa.d2k.modules.core.datatype.table.*;
public class DecisionTreeInducer extends FunctionInducer {
  int numInputs;
  int numOutputs;
  String [] inputNames;
  String [] outputNames;
...
//luckily, for now I can assume that at least the train test set are independent;
  ...
  public void instantiateBias(double [] bias) {
    MinDecompositionPopulation = (int) bias[0];
    MinErrorReduction          =       bias[1];


    UseSimpleBooleanSplit   = false;
    if (bias[2] > 0.5) UseSimpleBooleanSplit   = true;
    UseMidPointBasedSplit   = false;
    if (bias[3] > 0.5) UseMidPointBasedSplit   = true;
    UseMeanBasedSplit       = false;
    if (bias[4] > 0.5) UseMeanBasedSplit       = true;
    UsePopulationBasedSplit = false;
    if (bias[5] > 0.5) UsePopulationBasedSplit = true;
    SaveNodeExamples = false;
    if (bias[6] > 0.5) SaveNodeExamples = true;
    UseMeanNodeModels = false;
    if (bias[7] > 0.5) UseMeanNodeModels = true;
    UseLinearNodeModels = false;
    if (bias[8] > 0.5) UseLinearNodeModels = true;
  }
