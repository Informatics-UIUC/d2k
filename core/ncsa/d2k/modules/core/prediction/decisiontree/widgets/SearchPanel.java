...
public class SearchPanel extends JPanel implements ActionListener {
	...
	public SearchPanel(TreeScrollPane scrollpane, JD2KFrame parent) {
		...
		model = treescrollpane.getViewableModel();
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (source == populationadd) {
			...
			if (searchlist.size() > 0) {
				...
				ViewNode node = (ViewNode) searchlist.get(searchindex);
	...
	void updateViewport(ViewNode node) {
		double xnode = node.x;
		double ynode = node.y;

		double nodeheight = node.gheight;
		double nodewidth = node.gwidth;
	...
	void searchTree(Condition condition, ViewNode node) {
		boolean evaluation;

		if (node.isLeaf()) {
			evaluation = condition.evaluate(condition, node);
			node.search = evaluation;
	...
	protected class Condition {
		...
		boolean evaluate(Condition condition, ViewNode node) {
			boolean expression;
