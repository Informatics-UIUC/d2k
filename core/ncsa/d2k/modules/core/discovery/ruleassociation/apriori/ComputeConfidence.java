...
public class ComputeConfidence extends ncsa.d2k.core.modules.ComputeModule{
	...
	*/
	...
	/** debug option **/
	private boolean debug = true; // FIX THIS, CHANGE TO FALSE
	...
	public void doit () throws Exception {
		...
		if (debug) {
			...
			System.out.println ("ComputeConfidence-> number of examples: "+numExamples);
                ...
                // Compute Confidence Optimization
                //
                // Here begins actually computing the confidences
                // it uses other itemsets instead of counting subsets from the examples
                //
                // inefficiency:  this version necessitates one scan of the
                // examples, but this can be removed if a count per
                // item is taken when calculating the itemFlags
                //
                // inefficency:  doesn't take advantage of algorithm specific scan order
                //
                // inefficency:  doesn't break out of as many loops as possible
                int itemCounts[] = new int[itemFlags[0].length];
                for(int exampleIndex =0;exampleIndex < numExamples;exampleIndex++)
                {
                  for(int itemIndex = 0;itemIndex < itemFlags[0].length;itemIndex++)
                  {
                    if(itemFlags[exampleIndex][itemIndex])
                    {
                ...
                for(int itemsetIndex = 0;itemsetIndex < fis.length;itemsetIndex++)
                {
                  if ( showProgress )
                  {
                    if ((itemsetIndex % 5000) == 0)
                    {
                      System.out.println("Processed " + itemsetIndex + " rules out of " + numFis + ".");
                    }
                  }
                  for(int consequentIndex = 0;consequentIndex < fis[itemsetIndex].length - 1;consequentIndex++)
                  {
                    int [] newRule = new int [fis[itemsetIndex].length+1];

                    if(fis[itemsetIndex].length == 3)
                    {
                      int itemIndex = 1 - consequentIndex;
                      double c = ((double) fis[itemsetIndex][2])/itemCounts[fis[itemsetIndex][itemIndex]];
                      if(c * 100 >= confidence)
                      {

                        newRule[0] = fis[itemsetIndex][itemIndex];
                        newRule[1] = fis[itemsetIndex][consequentIndex];
                        newRule[2] = fis[itemsetIndex][2];

                        newRule[fis[itemsetIndex].length] = (int) (c * 10000.0);
                        finalRules.addElement (newRule);
                      }
                    }
                    else
                    {
                        for (int innerItemsetIndex = 0;
                             innerItemsetIndex < fis.length;
                             innerItemsetIndex++)
                        {
                          if (fis[itemsetIndex].length != fis[innerItemsetIndex].length + 1)
                          {
                            // proceed to next ancedent
                            continue;
                          }

                          for (int itemIndex = 0;
                               itemIndex < fis[itemsetIndex].length - 1;
                               itemIndex++)
                          {
                            if (itemIndex > consequentIndex)
                            {
                              newRule[itemIndex-1] = fis[itemsetIndex][itemIndex];

                              if (fis[itemsetIndex][itemIndex] !=
                                  fis[innerItemsetIndex][itemIndex - 1])
                              {
                                // proceed to next ancedent
                                break;
                              }
                              // have a match, check confidence
                              else if(fis[itemsetIndex].length == itemIndex + 2)
                              {

                                double c = ((double) fis[itemsetIndex][itemIndex + 1]) / ((double) fis[innerItemsetIndex][itemIndex]);
                                if(c * 100 >= confidence)
                                {
                                  newRule[fis[itemsetIndex].length-1] = fis[itemsetIndex][fis[itemsetIndex].length-1];
                                  newRule[fis[itemsetIndex].length] = (int) (c * 10000.0);
                                  finalRules.addElement (newRule);
                                }
                                // proceed to next ancedent
                            ...
                            else if (itemIndex < consequentIndex)
                            {
                              newRule[itemIndex] = fis[itemsetIndex][itemIndex];
                              if (fis[itemsetIndex][itemIndex] != fis[innerItemsetIndex][itemIndex])
                              {
                                // proceed to next ancedent
                                break;
                              }
                            ...
                            else // itemIndex == consequentIndex
                            {
                                newRule[newRule.length-3] = fis[itemsetIndex][consequentIndex];
                                if(fis[itemsetIndex].length == itemIndex + 2)
                                {
                                  newRule[fis[itemsetIndex].length-1] = fis[itemsetIndex][fis[itemsetIndex].length-1];
                                  double c = ((double) fis[itemsetIndex][itemIndex + 1]) / ((double) fis[innerItemsetIndex][itemIndex]);
                                  if(c * 100 >= confidence)
                        ...
                        if ( showProgress )
                        {
                          ...
                          System.out.println("Processed " + numFis + " rules out of " + numFis + ".");

                        }
                        // End of the Compute Confidence Optimization
